...
describe("extractRoutes", () => {
  it("extracts router.METHOD(path) calls from a route file", () => {
    ...
    const result = extractRoutes(source, "server/routes/auth.js")
    expect(result).toEqual([
      { method: "GET", path: "/check", file: "server/routes/auth.js" },
      { method: "POST", path: "/login", file: "server/routes/auth.js" },
    ])
  })
})
...
describe("extractMounts", () => {
  it("pairs app.use mount prefixes with the file imported under that name", () => {
    ...
    expect(extractMounts(source)).toEqual([
      ...
      { prefix: "/api/briefing", file: "./routes/briefing/index.js" },
    ])
  })
