...
function Bucket({ row, items, kind, accent, onActivateItem, todayYmd, now }: BucketProps) {
...
export interface AlfredBreakdownProps {
  kind: AlfredItemKind;
  title: string;
  caption?: string;
  total: number;
  buckets: Array<{ label: string; count: number; items: AlfredRow[] }>;
  accent: string;
  onActivateItem?: (action: AlfredChipAction) => void;
}

function AlfredBreakdown({ kind, title, caption, total, buckets, accent, onActivateItem }: AlfredBreakdownProps) {
