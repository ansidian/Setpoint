...
- `AlfredMessages.tsx` — UserLine, ToolRows, SayBlock (`preamble`/streaming → quiet prose; `done && !preamble` → serif lead), ErrorLine, SuggestionList (message leaves are React.memo'd: untouched messages stay referentially stable so token streaming only re-renders the active say block)
