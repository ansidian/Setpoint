...
function injectIntoHead(docHtml, headHtml) {
  if (HEAD_OPEN.test(docHtml)) {
    return docHtml.replace(HEAD_OPEN, (match) => match + headHtml);
  }
  if (HTML_OPEN.test(docHtml)) {
    return docHtml.replace(HTML_OPEN, (match) => `${match}<head>${headHtml}</head>`);
  }
...
export function withEmailContentSecurityPolicy(docHtml = "", policy = STRICT_EMAIL_CSP_POLICY) {
