...
import type { AddTaskPanelHost, PanelPosition } from "./types";
...
export function buildContainerStyle({
  ...
  if (isMobile) {
    return {
      ...
      width: "100%",
      maxHeight: "calc(88vh - var(--sp-safe-bottom))",
      overflowY: "auto",
...
export function buildInlineContainerStyle(_options?: { active?: boolean }): CSSProperties {
  return {
    display: "flex",
...
export function buildDropdownRowStyle(isMobile: boolean): CSSProperties {
  return {
    display: "flex",
