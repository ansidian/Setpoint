...
function formatTime(time: TimeParts | null | undefined) {
  if (!time) return "";
  let hour = time.hour;
...
function dateFromYmd(value: string | null | undefined) {
...
function formatTodoistResolvedDueString(resolved: ResolvedDate | null) {
...
function time24FromTime(time: TimeParts | null) {
  if (!time) return null;
...
function formatTodoistTime(value: string | null | undefined) {
  return formatDraftTime(value).replace(/\s/g, "").toLowerCase();
}

function buildRecurringDisplay(recurrenceDraft: TodoistRecurrenceDraft, includeTime = false) {
...
function formatTodoistWeekdays(weekdays: string[] | undefined) {
  const codes = Array.isArray(weekdays) ? weekdays : [];
  if (codes.join(",") === "MO,TU,WE,TH,FR") return "weekday";
  if (codes.join(",") === "SA,SU") return "weekend";
  return codes
    .map((code) => TODOIST_WEEKDAY_BY_CODE[code as keyof typeof TODOIST_WEEKDAY_BY_CODE])
    .filter(Boolean)
    .join(", ");
}

function buildTodoistRecurringDueString(recurrenceDraft: TodoistRecurrenceDraft, includeTime = false) {
...
function parseRecurringDue(input: string) {
  ...
  const recurrenceDraft = parsed.recurrenceDraft as TodoistRecurrenceDraft;
  const hasExplicitTime = !!parsed.parsedDateTime?.hasTime;
  const dueString = buildTodoistRecurringDueString(recurrenceDraft, hasExplicitTime)
    || parsed.matchedText
  ...
  return {
    ...
    dueString,
    recurrenceDraft,
    recurrenceSummary: buildRecurringDisplay(recurrenceDraft, hasExplicitTime),
  };
}

function getNextDayOfWeek(dayName: string, fromDate: Date) {
...
function resolveDate(
  input: string,
  { seededDueDate = null }: { seededDueDate?: string | null } = {},
): (ResolvedDate & { phrase: string }) | null {
  ...
  for (const { re, type } of DATE_TIME_PATTERNS) {
    ...
    switch (type) {
      ...
      case "iso": {
        const [year = 0, month = 0, day = 0] = match[1]!.split("-").map(Number);
...
export function formatResolvedDate(resolved: Pick<ResolvedDate, "date" | "time"> | null) {
...
export function parseTokens(
  ...
  const result: ParsedTodoistTokens = {
    priority: null,
    project: null,
    labels: [],
    datePhrase: null,
  ...
  if (priorityMatch) {
    result.priority = (priorityMatch[2] ? parseInt(priorityMatch[2], 10) : 1) as Exclude<TodoistPriority, null>;
...
export async function parseTokensWithChrono(
  input: string,
  projects: TodoistProject[],
  labels: TodoistLabel[],
  options: { seededDueDate?: string | null } = {},
