...
import type { ManualDue, TodoistEditorTask } from "./types";
...
export function formatTodoistDueStringFromEpoch(epochMs: number) {
...
export function formatTodoistDueDisplayFromEpoch(epochMs: number) {
  const { year, month, day, hour, minute } = laComponents(epochMs);
...
export function buildManualDue(epochMs: number): ManualDue {
  return {
    epochMs,
...
export function getInitialDueEpoch(editingTask: TodoistEditorTask | null | undefined) {
  if (!editingTask?.due_date) return null;
  const [year = 0, month = 0, day = 0] = editingTask.due_date.split("-").map(Number);
...
export function displayTimeToInputValue(dueTime: string | null | undefined) {
...
export function buildSeededDue(initialDueDate: string | null | undefined) {
  if (!initialDueDate) return null;
  const [year = 0, month = 0, day = 0] = String(initialDueDate).split("-").map(Number);
  if (!year || !month || !day) return null;
...
export function buildSeededDueDisplay({
  dueDate,
  dueTime,
  seededCreateDue,
}: {
  dueDate?: string | null;
  dueTime?: string | null;
  seededCreateDue?: Pick<ManualDue, "display"> | null;
}) {
  if (dueDate) {
