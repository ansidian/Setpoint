...
import type {
  ...
  ManualDue,
  ParsedTodoistTokens,
  ParsedTodoistTokensInput,
  TodoistEditorTask,
  TodoistNamedReference,
} from "./types";

// Manual labels win once the user has touched labels; otherwise the parsed
// @label tokens, falling back to an empty list.
export function resolveAddTaskLabels<T extends TodoistNamedReference>({
  useManualLabels,
  manualLabels,
  parsedLabels,
}: {
  useManualLabels?: boolean;
  manualLabels: T[] | null;
  parsedLabels: T[];
}): T[] {
...
export function resolveAddTaskDue({
  ...
  manualDue: Partial<ManualDue> | null;
  parsed: ParsedTodoistTokensInput;
  seededCreateDue: Partial<ManualDue> | null;
  seededDueDisplay: string | null;
  seededDueEpoch: number | null;
}) {
  ...
  const recurrenceSummary = !useManualDue ? parsed.recurrenceSummary || null : null;
...
export function buildAddTaskDraftPreview({
  ...
  manualDueEpochMs: number | null;
  useManualDue?: boolean;
  parsedDuePreview: ParsedTodoistTokens["duePreview"];
  parsedStripped: string;
  seededDueEpoch: number | null;
  editingDueDate?: string | null;
  editingDueTime?: string | null;
  editingTitle?: string;
  isEdit: boolean;
  input: string;
  resolvedPriority: number | null;
}): AddTaskDraftPreview | null {
...
export function buildOriginalDueValue({
  editingTask,
  seededDueString,
}: {
  editingTask?: TodoistEditorTask | null;
  seededDueString?: string | null;
}) {
  return editingTask
...
// `snapshot !== baseline`.
export function buildAddTaskDirtyBaseline({
  editingTask,
  originalDueValue,
}: {
  editingTask?: TodoistEditorTask | null;
