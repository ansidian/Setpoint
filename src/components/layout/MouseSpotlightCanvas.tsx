...
function createPatternTile(dpr: number): HTMLCanvasElement | null {
...
function createMaskCanvas(dpr: number): HTMLCanvasElement | null {
...
interface DrawSpotlightOptions {
  ctx: CanvasRenderingContext2D;
  canvas: HTMLCanvasElement;
  patternTile: HTMLCanvasElement;
...
export default function MouseSpotlightCanvas() {
  ...
  useEffect(() => {
    ...
    function rebuildResources(): boolean {
      dpr = Math.max(1, Math.min(window.devicePixelRatio || 1, MAX_DPR));
      canvasElement.width = SPOTLIGHT_SIZE * dpr;
      canvasElement.height = SPOTLIGHT_SIZE * dpr;
      ctx = canvasElement.getContext("2d", { alpha: true, desynchronized: true });
      if (!ctx) return false;
    ...
    function applyFrame() {
      ...
      if (visible && moved) {
        drawSpotlight({ ctx: ctx!, canvas: canvasElement, patternTile: patternTile!, maskCanvas: maskCanvas!, dpr, left, top });
      }

      if (moved) {
        canvasElement.style.transform = `translate3d(${left}px, ${top}px, 0)`;
        lastLeft = left;
        lastTop = top;
      }

      if (visibilityChanged) {
        canvasElement.style.opacity = visible ? String(SPOTLIGHT_OPACITY) : "0";
        lastVisible = visible;
      }
    }
