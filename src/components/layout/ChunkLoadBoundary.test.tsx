...
describe("ChunkLoadBoundary", () => {
  ...
  it("recognizes browser dynamic import failures", () => {
    expect(isDynamicImportLoadError(new TypeError("Failed to fetch dynamically imported module: https://ea.andysu.tech/assets/CalendarModal-old.js"))).toBe(true);
  ...
  it("reloads once when a lazy chunk fails to load", async () => {
    ...
    render(
      <ChunkLoadBoundary reloadPage={reloadPage} storage={storage} now={() => 1000}>
        <ThrowError error={new TypeError("Failed to fetch dynamically imported module: https://ea.andysu.tech/assets/CalendarModal-old.js")} />
      </ChunkLoadBoundary>,
    );
