...
vi.mock("@/components/ui/select", () => import("./selectMock.test-utils.jsx"));

const { default: ProviderModelSelect } = await import("./ProviderModelSelect.jsx");
...
describe("ProviderModelSelect", () => {
  ...
  it("falls back to the provider default when the model id is not in the provider", () => {
    ...
    expect(screen.getByLabelText("AI model").value).toBe("claude-sonnet-4-6");
  });
  ...
  it("keeps the current model id when it belongs to the selected provider", () => {
    ...
    expect(screen.getByLabelText("AI model").value).toBe("claude-haiku-4-5");
  });
  ...
  it("disables the model select when the selected provider is unavailable", () => {
    ...
    expect(screen.getByLabelText("AI model").disabled).toBe(true);
  });
  ...
  it("falls back to the first provider when the provided provider is unknown", () => {
    ...
    expect(screen.getByLabelText("AI provider").value).toBe("anthropic");
    expect(screen.getByLabelText("AI model").value).toBe("claude-sonnet-4-6");
  ...
  it("disables both selects when disabled is set", () => {
    ...
    expect(screen.getByLabelText("AI provider").disabled).toBe(true);
    expect(screen.getByLabelText("AI model").disabled).toBe(true);
  });
});
