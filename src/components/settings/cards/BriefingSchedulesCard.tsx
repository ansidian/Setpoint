...
import type { BriefingSchedule } from "../../../../shared/types/settings";

export default function BriefingSchedulesCard({ settings, setSettings, patch }: SettingsCardStateProps) {
  const [editingTimeIdx, setEditingTimeIdx] = useState<number | null>(null);
  const [frozenOrder, setFrozenOrder] = useState<string | null>(null);
  const schedContainerRef = useRef<HTMLDivElement | null>(null);
  const schedRectsRef = useRef<Record<string, number>>({});
  const prevSortOrderRef = useRef<string | null>(null);
  ...
  function captureRects() {
    ...
    schedContainerRef.current
      .querySelectorAll<HTMLElement>("[data-sched-idx]")
      .forEach((element) => {
        if (element.dataset.schedIdx) schedRectsRef.current[element.dataset.schedIdx] = element.getBoundingClientRect().top;
      });
  }

  const schedules = settings?.schedules || [];
  type IndexedSchedule = BriefingSchedule & { _oi: number };
  const items: IndexedSchedule[] = [...schedules].map((schedule, index) => ({ ...schedule, _oi: index }));
  ...
  const visibleSchedules = editingTimeIdx !== null && frozenOrder
    ? frozenOrder
        ...
        .filter((schedule): schedule is IndexedSchedule => schedule != null)
    : sortedItems;
            ...
            >
              <div className="flex min-w-0 items-start justify-between gap-3">
                <input
                  ...
                  onFocus={() => {
                    labelDraftRef.current = schedules?.[originalIndex]?.label || "";
                  }}
                  onBlur={() => {
                    const current = schedules?.[originalIndex]?.label || "";
                    if (current.trim()) {
                ...
                <button
                  ...
                  onClick={() => {
                    captureRects();
                    const updated = schedules.filter((_, index) => index !== originalIndex);
                    setSettings((current) => ({ ...(current || {}), schedules: updated }));
