...
import type { ApiTokenMetadata } from "../../../../shared/types/accounts";

interface PendingConfirmation { message: string; resolve: (value: boolean) => void }
interface NewTokenState { token: string; label: string; expiresAt: number }
...
export default function ApiTokensCard() {
  const [tokens, setTokens] = useState<ApiTokenMetadata[] | null>(null);
  ...
  useEffect(() => {
    listApiTokens()
      .then(setTokens)
      .catch((error) => setLoadError(errorMessage(error, "Failed to load tokens")));
  }, []);
  ...
  async function handleCreate(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setCreateError(errorMessage(error, "Failed to create token"));
    } finally {
      setCreating(false);
    }
  }

  async function handleRevoke(id: string | number) {
