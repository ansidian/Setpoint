...
import type { BillPayBehaviorTargets } from "../../../../shared/types/settings";
import type { SettingsCardStateProps } from "../settingsTypes";
import type {
  NormalizedBillPayBehavior,
  NormalizedBillPayMappings,
  NormalizedBillPayProfile,
  StoredActualOption,
} from "./billPayMappingsModel";

interface BehaviorEditorProps {
  behavior: NormalizedBillPayBehavior;
  index: number;
  count: number;
...
function BehaviorEditor({
  ...
  function updateTargets(updater: (targets: BillPayBehaviorTargets) => BillPayBehaviorTargets) {
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="grid gap-3 md:grid-cols-3">
          <MappingSelect
            ...
            onChange={(type) => onChange({ ...behavior, type: type as NormalizedBillPayBehavior["type"] })}
          />
          <MappingSelect
            label="Amount strategy"
            value={behavior.amountStrategy}
            options={AMOUNT_STRATEGIES}
            onChange={(amountStrategy) => onChange({ ...behavior, amountStrategy: amountStrategy as NormalizedBillPayBehavior["amountStrategy"] })}
          />
          <MappingSelect
            label="Amount fallback"
            value={behavior.amountFallback}
            options={AMOUNT_FALLBACKS}
            onChange={(amountFallback) => onChange({ ...behavior, amountFallback: amountFallback as NormalizedBillPayBehavior["amountFallback"] })}
          />
        </div>
...
interface ProfileEditorProps {
  profile: NormalizedBillPayProfile;
  index: number;
  count: number;
  collapsed: boolean;
  accounts: StoredActualOption[];
...
function ProfileEditor({
  ...
  function updateBehavior(behaviorIndex: number, updater: (behavior: NormalizedBillPayBehavior) => NormalizedBillPayBehavior) {
    onChange({
      ...profile,
...
export default function BillPayMappingsCard({
  ...
  function applyMappings(nextMappings: NormalizedBillPayMappings) {
    setSettings((current) => ({ ...(current || {}), bill_pay_mappings: nextMappings }));
    patch({ bill_pay_mappings: nextMappings });
  }

  function updateProfile(profileIndex: number, updater: (profile: NormalizedBillPayProfile) => NormalizedBillPayProfile) {
    applyMappings({
      ...mappings,
...
const EMPTY_RESET: NormalizedBillPayMappings = { version: 1, profiles: [] };
