...
import type { ImportantSender } from "../../../../shared/types/settings";

export default function ImportantSendersCard() {
  const [importantSenders, setImportantSenders] = useState<ImportantSender[]>([]);
  ...
  async function persistSenders(nextSenders: ImportantSender[]) {
    ...
    try {
      ...
      setSenderError(err instanceof Error ? err.message : "Could not save important senders.");
    } finally {
      ...
      <div className="flex flex-col gap-3">
        ...
        <form
          onSubmit={(event: FormEvent<HTMLFormElement>) => {
            ...
            const input = event.currentTarget.elements.namedItem("senderEmail") as HTMLInputElement | null;
            if (!input) return;
            ...
            const nextSenders = [
              ...importantSenders,
              { address, name: address.split("@")[0], source: "manual" as const },
            ];
            input.value = "";
            persistSenders(nextSenders);
          }}
