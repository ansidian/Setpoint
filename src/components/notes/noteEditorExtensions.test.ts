...
import { toggleCheckboxLine, makeTagCompletionSource, toggleMarkerWrap, linkInsertion, checkboxEnterAction, checkboxAutoConvert } from "./noteEditorExtensions";

// Apply toggleMarkerWrap to a doc + [anchor, head] selection, return [text, selFrom, selTo].
function applyWrap(doc: string, anchor: number, head: number, marker: string): [string, number, number] {
...
describe("makeTagCompletionSource", () => {
  ...
  const ctxFor = (text: string) => new CompletionContext(EditorState.create({ doc: text }), text.length, false);
