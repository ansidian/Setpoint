...
import { buildNoteEditorExtensions } from "./noteEditorExtensions";
...
export interface NoteEditorProps {
  ...
  placeholder?: string;
  submitOnEnter?: boolean;
  editorApiRef?: RefObject<NoteEditorApi | null>;
  ariaLabel?: string;
}
...
export default function NoteEditor({
  ...
  const onBlurRef = useRef<NoteEditorProps["onBlur"]>(undefined);
  ...
  const cbRef = useRef<NoteEditorCallbacks>({});
  cbRef.current = {
    onChange: (v: string) => { lastEmittedRef.current = v; onChange?.(v); },
    onSubmit,
    onCancel,
    submitOnEnter,
    getTags: () => tags,
