...
import type { NoteEditorCallbacks } from "./NoteEditor";
...
export function toggleCheckboxLine(content: unknown, index: number): string {
...
export function toggleMarkerWrap(state: EditorState, marker: string) {
...
export function linkInsertion(selText: string): { text: string; caret: number } {
  const sel = selText || "";
...
// has no underline, and the rendered subset (renderNoteMarkdown.tsx) has no token
...
export function checkboxEnterAction(lineText: string): { type: "exit" } | { type: "continue"; insert: string } | null {
...
export function checkboxAutoConvert(beforeCursor: string): { prefix: string } | null {
...
const checkboxEnterCommand: Command = (view: EditorView) => {
  const { state } = view;
...
const checkboxConvertCommand: Command = (view: EditorView) => {
  const { state } = view;
  if (state.selection.ranges.length !== 1) return false;
  const range = state.selection.main;
  if (!range.empty) return false;
...
export function makeTagCompletionSource(getTags: () => string[]): (context: CompletionContext) => CompletionResult | null {
...
const STYLED_PARENTS: Record<string, string> = { StrongEmphasis: "cm-note-strong", Emphasis: "cm-note-em", InlineCode: "cm-note-code" };
...
export const livePreview = ViewPlugin.fromClass(
  class {
    ...
    update(u: ViewUpdate) { if (u.docChanged || u.selectionSet || u.viewportChanged) this.decorations = this.build(u.view); }
...
export function noteTheme(): Extension {
...
class CheckboxWidget extends WidgetType {
  ...
  constructor(checked: boolean, markerPos: number) {
    super();
  ...
  override eq(other: CheckboxWidget): boolean { return other.checked === this.checked && other.markerPos === this.markerPos; }
...
interface NoteEditorCallbacksRef { current: NoteEditorCallbacks }

export function buildNoteEditorExtensions({ callbacksRef, placeholderText }: { callbacksRef: NoteEditorCallbacksRef; placeholderText: string }): Extension[] {
