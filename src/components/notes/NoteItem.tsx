...
import { parseTags, stripTags, noteEditedAge } from "./notesModel";
import NoteContextMenu from "./NoteContextMenu";
import NoteEditor from "./NoteEditor";
import type { Note, NoteId } from "../../../shared/types/notes";

interface NoteMenuPosition { x: number; y: number }

export interface NoteItemProps {
  note: Note;
  accent?: string;
  onUpdate: (id: NoteId, content: string) => void;
  onDelete: (id: NoteId) => void;
  onArchive?: (id: NoteId) => void;
  onUnarchive?: (id: NoteId) => void;
  onPromote?: (note: Note, anchorRef: RefObject<HTMLButtonElement | null>) => void;
...
export default function NoteItem({
  ...
  const [menu, setMenu] = useState<NoteMenuPosition | null>(null);
  ...
  const commitEditWith = useCallback((v?: string) => {
  ...
  const handlePointerDownCapture = useCallback((event: PointerEvent<HTMLDivElement>) => {
    const interactive = (event.target as Element).closest("a, button, textarea, input, .cm-editor");
  ...
  const handleRowClick = useCallback((e: MouseEvent<HTMLDivElement>) => {
