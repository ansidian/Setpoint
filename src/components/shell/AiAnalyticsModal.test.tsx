...
const AiAnalyticsModal = (await import("./AiAnalyticsModal")).default;
...
it("isolates a failing section", async () => {
  ...
  vi.mocked(getAlfredUsageStats).mockRejectedValueOnce(new Error("boom"));
  render(<AiAnalyticsModal open onClose={() => {}} />);
...
it("recovers the failing section when Try again succeeds", async () => {
  ...
  vi.mocked(getAlfredUsageStats).mockReset();
  vi.mocked(getAlfredUsageStats)
    .mockRejectedValueOnce(new Error("boom"))
    .mockResolvedValue({ queries: 5, tools: { totalCalls: 3, byTool: [], distinctTools: 0 }, byModel: {}, cacheHitRate: 0.5 } as unknown as Awaited<ReturnType<typeof getAlfredUsageStats>>);
...
it("does not throw or warn when closed before the fetch resolves", async () => {
  ...
  vi.mocked(getAlfredUsageStats).mockReset();
  vi.mocked(getAlfredUsageStats).mockResolvedValue({ queries: 5, tools: { totalCalls: 3, byTool: [], distinctTools: 0 }, byModel: {}, cacheHitRate: 0.5 } as unknown as Awaited<ReturnType<typeof getAlfredUsageStats>>);
