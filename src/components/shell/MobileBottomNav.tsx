import { CalendarDays, Inbox, LayoutList, Wallet } from "lucide-react";
import type { DashboardTab } from "../dashboard/dashboardShellModel";

type MobileDashboardTab = Exclude<DashboardTab, "news" | "notes">;

const NAV_TABS = ["dashboard", "inbox", "calendar", "finances"] as const satisfies readonly MobileDashboardTab[];
const NAV_ICONS = { dashboard: LayoutList, inbox: Inbox, calendar: CalendarDays, finances: Wallet };
const NAV_LABELS = { dashboard: "Dashboard", inbox: "Inbox", calendar: "Calendar", finances: "Finances" };
...
interface MobileBottomNavProps {
  ...
  onRetap?: (tab: MobileDashboardTab) => void;
  inboxUnreadSignalCount: number;
}

export function MobileBottomNav({ tab, onTab, onRetap, inboxUnreadSignalCount }: MobileBottomNavProps) {
  return (
    <nav
      aria-label="Primary"
