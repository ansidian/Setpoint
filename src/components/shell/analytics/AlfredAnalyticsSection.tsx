...
import { Metric } from "./analyticsPrimitives";
import type { MetricProps } from "./analyticsPrimitives";
import { formatPercent, formatUsdEstimate, formatCompactNumber, numberValue, formatModelName } from "./analyticsFormat";
import type { AlfredUsageModelSummary, AlfredUsageStats, AlfredUsageToolSummary } from "../../../../shared/types/alfred";

type AlfredAnalyticsStats = Partial<Omit<AlfredUsageStats, "byModel" | "tools" | "comparisonWindows">> & {
  comparisonWindows?: { monthToDate?: object };
  byModel?: Record<string, Partial<AlfredUsageModelSummary> | undefined>;
  tools?: {
    totalCalls?: number;
    distinctTools?: number;
    byTool?: Array<Partial<AlfredUsageToolSummary> & { name: string }>;
  };
};
...
export default function AlfredAnalyticsSection({ stats }: { stats: AlfredAnalyticsStats }) {
  ...
  const models = Object.entries(stats?.byModel || {}).filter((entry): entry is [string, NonNullable<typeof entry[1]>] => Boolean(entry[1]));
