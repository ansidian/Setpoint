...
function emptyEventsData(overrides: Record<string, unknown> = {}) {
  return {
    editable: true,
    getEvents: () => [],
    ...overrides,
...
function miniDate(name: string | RegExp) {
...
describe("CalendarModal Mini Calendar activation", () => {
  it("selects a Mini Calendar date and lands the agenda rail on that date", async () => {
    ...
    HTMLElement.prototype.getBoundingClientRect = function getBoundingClientRectMock() {
      ...
      if (
        ...
        return { top: 760, bottom: 794, left: 0, right: 280, width: 280, height: 34 } as DOMRect;
      }
  ...
  it("syncs Mini Calendar selection from agenda scrolling before later date activation", async () => {
    ...
    const scrollTo = vi.fn(function scrollToMock(this: HTMLElement, options: ScrollToOptions) {
      if (Number.isFinite(options?.top)) this.scrollTop = options.top ?? 0;
    });
    ...
    try {
      ...
      HTMLElement.prototype.getBoundingClientRect = function getBoundingClientRectMock() {
        ...
        if (this.getAttribute?.("data-date-key") === "2026-05-01") {
          return { top: -420, bottom: -386, left: 0, right: 280, width: 280, height: 34 } as DOMRect;
        }
        ...
        if (this.getAttribute?.("data-date-key") === "2026-05-15") {
          return { top: -60, bottom: -26, left: 0, right: 280, width: 280, height: 34 } as DOMRect;
        }
        if (this.getAttribute?.("data-date-key") === "2026-05-20") {
          return { top: 2, bottom: 36, left: 0, right: 280, width: 280, height: 34 } as DOMRect;
        }
        if (this.getAttribute?.("data-date-key") === "2026-05-28") {
          return { top: 760, bottom: 794, left: 0, right: 280, width: 280, height: 34 } as DOMRect;
        }
        if (this.getAttribute?.("data-date-key")) {
          return { top: 9999, bottom: 9999, left: 0, right: 280, width: 280, height: 34 } as DOMRect;
        }
