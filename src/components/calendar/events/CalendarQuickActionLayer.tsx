...
import {
  ...
  focusMenuColor,
  menuStyle,
} from "./quickActionMenuLayout";
import { checkColorForDot, selectedEventColorId } from "./quickActionColorModel";
import type { GoogleEventColor } from "../../../../shared/calendar-event-colors";
import type { CalendarRecurrenceScope } from "../../../../shared/types/calendar";
import type useCalendarQuickActions from "./useCalendarQuickActions";

type CalendarQuickActionsController = ReturnType<typeof useCalendarQuickActions>;
type CalendarQuickActionsView = Pick<
  CalendarQuickActionsController,
  | "contextMenu"
  | "prompt"
  | "status"
  | "closeContextMenu"
  ...
  | "cancelPrompt"
>;
type QuickActionTone = "default" | "danger" | "primary";
...
interface QuickActionsProps {
  quickActions: CalendarQuickActionsView;
}
...
interface CalendarQuickActionLayerProps {
  quickActions: CalendarQuickActionsView | null;
}
...
function actionButtonColors(tone: QuickActionTone, active: boolean) {
  if (tone === "danger") {
    return {
...
function ActionButton({ children, tone = "default", disabled, onClick, testId }: ActionButtonProps) {
...
function ColorDotButton({ color, selected, disabled, onClick, scopeCount = 1 }: ColorDotButtonProps) {
  const [active, setActive] = useState(false);
...
function ColorGrid({ menu, quickActions }: QuickActionsProps & { menu: NonNullable<CalendarQuickActionsView["contextMenu"]> }) {
...
function ScopeButton({ option, selected, disabled, onClick }: ScopeButtonProps) {
  const [active, setActive] = useState(false);
  const interactive = active && !disabled;
...
function ContextMenu({ quickActions }: QuickActionsProps) {
...
function ScopePrompt({ quickActions }: QuickActionsProps) {
...
function Status({ quickActions }: QuickActionsProps) {
...
export default function CalendarQuickActionLayer({ quickActions }: CalendarQuickActionLayerProps) {
