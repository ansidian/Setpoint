...
export interface CalendarRecurrenceSummaryDraft {
  frequency?: string;
  interval?: number | string;
  weekdays?: string[];
}

export function toPacificYmd(epoch: number | string | Date) {
...
export function addMinutesToDraftDateTime(
...
] satisfies Array<{ value: CalendarRecurrenceScope; label: string; description: string }>;

export function recurringScopeLabel(scope?: CalendarRecurrenceScope | null) {
...
export function formatMonthDay(dateStr?: string | null) {
...
export function formatRecurrenceSummary(
  recurrenceDraft: CalendarRecurrenceSummaryDraft | null | undefined,
  startDate?: string | null,
) {
  if (!recurrenceDraft) return "";
  const { frequency, interval: rawInterval = 1, weekdays } = recurrenceDraft;
...
export function formatScheduleSummary(draft: CalendarScheduleDraft | null | undefined, fallback: string) {
...
export function previewSegmentStyle(kind: string): CSSProperties {
  return {
...
export function textFieldStyle({ invalid = false }: { invalid?: boolean } = {}): CSSProperties {
  return {
    width: "100%",
