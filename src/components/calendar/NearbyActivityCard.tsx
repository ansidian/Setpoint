...
import {
  ...
  formatNeighborDate,
  neighborActiveCount,
} from "./calendarOverviewModel.ts";
import type { ItemsByDay, NeighborActiveView } from "./calendarOverviewModel.ts";

interface NearbyModel { itemNoun?: string; accent: string }
...
export function NearbyActivityCard({
  ...
  onSelectDay,
  compact = false,
}: { model: NearbyModel; activeView?: NeighborActiveView | null; itemsByDay: ItemsByDay; selectedDay: number; viewYear: number; viewMonth: number; onSelectDay?: (day: number) => void; compact?: boolean }) {
