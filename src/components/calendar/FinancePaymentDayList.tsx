...
export default function FinancePaymentDayList({ date, payments, loading, unavailable, complete, onNavigate }: {
  ...
  onNavigate: (payment: FinancePayment) => void;
}) {
  ...
  return <div className="fac-payment-list" aria-label="Payments on this day">
    {rows.map(row => {
      ...
      return <button key={`${row.status}:${row.id}`} data-finance-detail-trigger aria-haspopup="dialog" onClick={() => onNavigate(row)}>
        <StatusIcon size={14} className={row.status === 'recorded' ? 'fin-paid' : 'fin-outflow'} aria-hidden="true"/>
        <span><strong>{row.name}</strong><small>{row.status === 'recorded' ? 'Recorded in Actual' : 'Scheduled'}{row.direction === 'transfer' ? ' · Transfer' : row.direction === 'income' ? ' · Inflow' : ''}</small></span>
