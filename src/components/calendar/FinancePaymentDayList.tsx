  ...
  return <div className="fac-payment-list" aria-label="Payments on this day">
    {rows.map(row => {
      ...
      return <button key={`${row.status}:${row.id}`} data-finance-detail-trigger onClick={event => onNavigate(row, event.currentTarget)}>
        ...
        <span><strong>{row.name}</strong><small>{row.status === 'recorded' ? row.direction === 'transfer' ? 'Transferred' : row.direction === 'income' ? 'Received' : 'Paid' : 'Scheduled'}{row.direction === 'transfer' ? ' · Transfer' : row.direction === 'income' ? ' · Inflow' : ''}</small></span>
