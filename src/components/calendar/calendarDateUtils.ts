...
export function pacificYMD(ms: unknown) {
  return PACIFIC_YMD_FORMATTER.format(ms instanceof Date ? ms : new Date(typeof ms === "number" ? ms : 0));
}

export function pacificTime24(ms: unknown) {
  return PACIFIC_TIME_FORMATTER_24.format(ms instanceof Date ? ms : new Date(typeof ms === "number" ? ms : 0));
}
...
export function addDaysYmd(value: string, delta: number): string {
...
export function getVisibleGridRange(year: number, month: number) {
