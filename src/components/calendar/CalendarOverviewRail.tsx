import { getOverviewModel } from "./calendarOverviewModel.ts";
import {
  EventsLoadingFrame,
  MetricCard,
} from "./CalendarRailPrimitives.tsx";
import { railContentStyle, railStaticStyle } from "./calendarRailStyles.ts";
import type { OverviewOptions } from "./calendarOverviewModel.ts";

export function CalendarOverviewRail(props: OverviewOptions) {
  const model = getOverviewModel(props)!;
  const loading = !!(props.data && typeof props.data === "object" && (props.data as { isLoading?: boolean }).isLoading);
