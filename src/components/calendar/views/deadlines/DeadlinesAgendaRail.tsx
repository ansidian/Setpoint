...
import { DeadlineStatusIcon } from "./DeadlineStatusIndicator.tsx";
import { buildDeadlinesAgendaGroups } from "./deadlinesAgendaModel.ts";
import type { AgendaDeadlineItem, DeadlinesAgendaGroup, DeadlinesAgendaMonthResult, DeadlinesAgendaResult } from "./deadlinesAgendaModel";
import { deadlineMatchesItemId } from "./deadlinesModel.ts";
import type { DeadlinesComputed } from "./deadlinesModel";
...
function groupDate(group: Pick<DeadlinesAgendaGroup, "dateKey">): Date | null {
...
function DeadlineRow({ task, selected, onSelect, quickActions }: {
  task: AgendaDeadlineItem;
  selected: boolean;
  onSelect: (task: AgendaDeadlineItem, element: HTMLElement) => void;
...
function EmptyDeadlineDay({ fallback }: { fallback: boolean }) {
  return (
    <div
      style={{
        padding: "12px 10px",
...
function CompletedToggle({ enabled, onToggle }: { enabled: boolean; onToggle: () => void }) {
  return (
    <div
      style={{
        flex: "0 0 auto",
...
function hasSelectedCompletedDeadline(agenda: DeadlinesAgendaResult, selectedItemId: unknown): boolean {
...
interface DeadlineActionPayload {
  item: AgendaDeadlineItem;
  dateKey: string;
  anchorElement: HTMLElement;
  sourceCellElement: HTMLElement;
  anchorKind: string;
}
export interface DeadlinesAgendaRailProps {
  viewYear: number;
  viewMonth: number;
  computed: DeadlinesComputed;
  ...
  onDeadlineAction?: (payload: DeadlineActionPayload) => void;
  onFilteredSelectedDeadlineHidden?: () => void;
  showCompleted?: boolean | null;
  onShowCompletedChange?: Dispatch<SetStateAction<boolean>>;
  deadlineQuickActions?: DeadlineQuickActions | null;
}
...
const DeadlinesAgendaRail = forwardRef(function DeadlinesAgendaRail({
...
}: DeadlinesAgendaRailProps, ref: ForwardedRef<AgendaMonthScrollHandle>) {
  ...
  const renderMonth = useCallback((month: AgendaScrollMonth, { registerHeader, registerSection, registerRow, registerContent }: AgendaRegistrationCallbacks) => {
    ...
    return (
    <AgendaRailShell
      groups={deadlineMonth.visibleGroups}
