...
export function resolveDeadlineRescheduleTarget(sourceDate?: string | null, targetDate?: string | null): string | null {
...
export function buildDeadlineReschedulePayload(sourceItem: { due_time?: string | null } | null | undefined, targetDate: string): { dueDate: string; dueTime?: string } {
  const payload: { dueDate: string; dueTime?: string } = { dueDate: targetDate };
...
export function deadlineDragAllowed(item: { itemKind?: string; recurring?: boolean; complete?: boolean } | null | undefined, dragEnabled: boolean): boolean {
  return !!dragEnabled
