...
import { colorWithAlpha, contrastText } from "./eventsAgendaColor.ts";
...
import type { AgendaEvent } from "./eventsAgendaModel";

export interface EventsAgendaQuickActions {
  dragEnabled?: boolean;
  isEventSelectionSelected?: (event: AgendaEvent) => boolean;
  beginDrag?: (event: AgendaEvent) => boolean;
  endDrag?: () => void;
  openContextMenu?: (input: { event: AgendaEvent; x: number; y: number }) => void;
  toggleEventSelection?: (input: {
    event: AgendaEvent;
    dateKey: string;
    anchorElement: HTMLElement;
    sourceCellElement: HTMLElement;
    anchorKind: "agenda-chip" | "agenda-row";
  }) => void;
}
interface AgendaEventRowBaseProps {
  event: AgendaEvent;
  selected: boolean;
  onSelect: (event: AgendaEvent, anchor: HTMLElement, anchorKind: "agenda-chip" | "agenda-row") => void;
  quickActions?: EventsAgendaQuickActions | null;
  onDirtyBlocked?: () => boolean;
  onPreviewStart?: (event: AgendaEvent) => void;
  onPreviewEnd?: (event: AgendaEvent) => void;
}

function agendaEventMatchItemIds(event: AgendaEvent): string {
  return [
    event?.agendaItemId,
    event?.id,
    event?.iCalUID,
...
function isPastTimedEventToday(event: AgendaEvent, dateKey: string, todayKey: string): boolean {
  return dateKey === todayKey && typeof event.endMs === "number" && event.endMs < Date.now();
}
...
export function TimedRow({
  ...
  onPreviewStart,
  onPreviewEnd,
}: AgendaEventRowBaseProps & { dateKey: string; todayKey: string }) {
