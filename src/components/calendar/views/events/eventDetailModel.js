...
export function sanitizeEventDisplayTitle(value) {
...
export function compactEventTimeRange(ev) {
  if (ev?.allDay) return ev.duration || "All day";
  const start = ev?.startMs ? pacificTime(ev.startMs) : null;
  const end = ev?.endMs ? pacificTime(ev.endMs) : null;
...
export function specialEventLabel(ev) {
...
export function isEditableEvent(ev) {
...
export function calendarActionUrl(ev) {
