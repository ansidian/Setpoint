...
import { buildMiniCalendarModel } from "./miniCalendarModel.ts";
import type { MiniCalendarMarker } from "./miniCalendarModel";
import type { CalendarItemLike } from "../calendarViewTypes";

type MiniCalendarModel = ReturnType<typeof buildMiniCalendarModel>;
type MiniCalendarCell = MiniCalendarModel["cells"][number];
...
function formatMonthParts(viewYear: number, viewMonth: number): { month: string; year: string } {
...
function dateTone(cell: MiniCalendarCell): "today" | "adjacent" | "current" {
  if (cell.isToday) return "today";
  if (!cell.inCurrentMonth) return "adjacent";
  return "current";
}

function dateFill(cell: MiniCalendarCell): string {
...
function dateTextColor(cell: MiniCalendarCell): string {
...
function dateCellBackground(cell: MiniCalendarCell): string {
...
function parseCssColor(value: unknown): RgbColor | null {
  ...
  if (hex) {
    const matchedHex = hex[1] || "";
    const raw = matchedHex.length === 3
      ? matchedHex.split("").map((part) => part + part).join("")
      : matchedHex;
    return {
...
function markerContrastRing(markerColor: string, backdropColor: string | null): { tone: "dark" | "light"; color: string } | null {
...
function dateAriaLabel(cell: MiniCalendarCell): string {
  return [
...
function Marker({ marker, backdropColor = null }: { marker: MiniCalendarMarker; backdropColor?: string | null }) {
...
export function AgendaRailWithMiniCalendar({ miniCalendar, children }: { miniCalendar: ReactNode; children: ReactNode }) {
  return (
    <div
...
export default function MiniCalendar({
  ...
  const markerBackdropColor = (cell: MiniCalendarCell): string | null => {
