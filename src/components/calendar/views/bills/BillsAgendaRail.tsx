...
} from "./billsAgendaModel.ts";
import type { AgendaFinanceItem, BillsAgendaCacheEntry, BillsAgendaGroup, BillsAgendaMonthResult } from "./billsAgendaModel";
import type { BillsComputed, BillsViewData } from "./billsModel";
import type { AgendaMonthScrollHandle, AgendaRegistrationCallbacks, AgendaScrollCommand, AgendaScrollMonth } from "../agenda/AgendaMonthScrollContainer";
...
function groupDate(group: Pick<BillsAgendaGroup, "dateKey">): Date | null {
...
function BillRow({ bill, selected, onSelect, onPreviewStart, onPreviewEnd }: {
  bill: AgendaFinanceItem;
  selected: boolean;
  onSelect: (bill: AgendaFinanceItem, element: HTMLElement) => void;
  onPreviewStart?: (bill: AgendaFinanceItem) => void;
  onPreviewEnd?: (bill: AgendaFinanceItem) => void;
}) {
...
function EmptyBillDay({ fallback, mobileAgenda, monthName }: { fallback: boolean; mobileAgenda: boolean; monthName?: string }) {
...
function previewSourceKey(item?: AgendaFinanceItem | null): string {
  return `${item?.agendaItemId || item?.id || ""}:${item?.agendaDateKey || item?.dateKey || ""}`;
}

function previewItemForBill(item: AgendaFinanceItem): AgendaFinanceItem {
  const dateKey = item.agendaDateKey || item.dateKey || "";
  return {
    ...item,
...
interface BillActionPayload {
  item: AgendaFinanceItem;
  dateKey: string;
...
export interface BillsAgendaRailProps {
  viewYear: number;
  viewMonth: number;
  computed: BillsComputed;
  ...
  onBillAction?: (payload: BillActionPayload) => void;
  getMonthBills?: ((year: number, month: number) => BillsViewData | null) | null;
  billsRange?: (AgendaRangeController & { loading?: boolean }) | null;
  dataRevision?: number;
...
const BillsAgendaRail = forwardRef(function BillsAgendaRail({
  ...
  dataRevision = 0,
  mobileAgenda = false,
}: BillsAgendaRailProps, ref: ForwardedRef<AgendaMonthScrollHandle>) {
  ...
  const monthGroupsCacheRef = useRef<Map<string, BillsAgendaCacheEntry> | null>(null);
  ...
  const [hoverPreviewItem, setHoverPreviewItem] = useState<AgendaFinanceItem | null>(null);
  ...
  function startHoverPreview(item: AgendaFinanceItem) {
    setHoverPreviewItem(previewItemForBill(item));
  }

  function endHoverPreview(item: AgendaFinanceItem) {
  ...
  const renderMonth = useCallback((month: AgendaScrollMonth, { registerHeader, registerSection, registerRow, registerContent }: AgendaRegistrationCallbacks) => {
    const billMonth = month as BillsAgendaMonthResult;
    return (
    <AgendaRailShell
      groups={billMonth.visibleGroups}
