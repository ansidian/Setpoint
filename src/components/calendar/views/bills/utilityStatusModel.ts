...
import { formatShortDate, TRACKED_UTILITIES } from "./billsModel.ts";
import type { BillScheduleInput, BillsViewData } from "./billsModel";
...
export function pickBestUtilityMatch(matches: BillScheduleInput[] | null | undefined, today: string): BillScheduleInput | null {
  if (!matches?.length) return null;
  let bestUpcoming: BillScheduleInput | null = null;
  let bestRecentPaid: BillScheduleInput | null = null;
  let fallback: BillScheduleInput | null = null;
...
export function deriveUtilityStatus(data: BillsViewData | null | undefined, today: string): UtilityStatus[] {
  ...
  const rows = TRACKED_UTILITIES.map((utility) => {
    const matches = schedules.filter((entry) => {
      ...
      const payeeName = payeeCond ? payeeMap[String(payeeCond.value)] : null;
    ...
    const conditionAmount = Number(amtCond?.value);
    const scheduleAmount = schedule?.amount == null || schedule.amount === "" ? null : Number(schedule.amount);
    const amount = amtCond?.value ? Math.abs(conditionAmount) / 100 : scheduleAmount;
...
export function deriveUtilityDateText(
  utility: Pick<UtilityStatus, "found" | "next_date"> & Partial<Pick<UtilityStatus, "isHonored" | "isStale">>,
): string {
