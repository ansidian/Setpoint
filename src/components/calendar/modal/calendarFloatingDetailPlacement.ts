...
export interface CalendarRectLike {
  readonly top: number;
...
export interface FloatingDetailPlacement {
  top: number;
  left: number;
  width: number;
  maxHeight: number;
  caretSide: FloatingDetailSide | null;
  caretTop: number;
}

type FloatingDetailBounds = CalendarRectLike;

interface PlacementCandidate {
  side: FloatingDetailSide;
  top: number;
  left: number;
  width: number;
  height: number;
  bounds: FloatingDetailBounds;
}

export interface ResolveFloatingDetailPlacementOptions {
  anchorRect?: CalendarRectLike | null;
  sourceRect?: CalendarRectLike | null;
  exclusionRect?: CalendarRectLike | null;
  calendarRect?: CalendarRectLike | null;
  railRect?: CalendarRectLike | null;
  panelHeight?: number;
  mode?: string;
  preferredSide?: FloatingDetailSide | null;
  forcedSide?: FloatingDetailSide | null;
  allowRailOverlap?: boolean;
}
...
function rectIntersects(a: CalendarRectLike | null | undefined, b: CalendarRectLike | null | undefined): boolean {
  if (!a || !b) return false;
  return a.left < b.right && a.right > b.left && a.top < b.bottom && a.bottom > b.top;
}

function overlapArea(a: CalendarRectLike | null | undefined, b: CalendarRectLike | null | undefined): number {
  if (!a || !b) return 0;
...
function normalizedBounds(calendarRect?: CalendarRectLike | null): FloatingDetailBounds {
  if (calendarRect) {
    return {
...
function panelRect({ left, top, width, height }: Pick<PlacementCandidate, "left" | "top" | "width" | "height">): CalendarRectLike {
  return {
    left,
    top,
    right: left + width,
...
function candidateScore(candidate: PlacementCandidate, exclusions: CalendarRectLike[]): number {
...
export function resolveFloatingDetailPlacement({
...
}: ResolveFloatingDetailPlacementOptions): FloatingDetailPlacement {
  ...
  const exclusions = [sourceRect, exclusionRect, railExclusion].filter((rect): rect is CalendarRectLike => Boolean(rect));
  const rightCandidate: PlacementCandidate = {
    side: "right",
    top,
  ...
  const leftCandidate: PlacementCandidate = {
    side: "left",
    top,
