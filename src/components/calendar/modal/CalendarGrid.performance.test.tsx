...
import type { CalendarGridActiveViewContract } from "./CalendarGrid";
import CalendarCellItemStack from "./CalendarCellItemStack";
import * as spanLayoutModule from "./calendarEventSpanLayout";
import * as calendarGridRowModelModule from "../../../hooks/calendar/calendarGridRowModel";
import type * as SpanLayoutModule from "./calendarEventSpanLayout";
import type * as CalendarGridRowModelModule from "../../../hooks/calendar/calendarGridRowModel";
...
import { renderBillsCellContents } from "../views/bills/BillsCellContent.tsx";
import {
...
} from "../views/bills/billsModel.ts";

vi.mock("./calendarEventSpanLayout", async (importOriginal) => {
  const actual = await importOriginal() as typeof SpanLayoutModule;
  return {
    ...actual,
...
vi.mock("./CalendarCellItemStack", () => ({
  default: vi.fn(({ dateKey }: ComponentProps<typeof CalendarCellItemStack>) => <div data-testid={`item-stack-${dateKey}`} />),
}));

vi.mock("../../../hooks/calendar/calendarGridRowModel", async (importOriginal) => {
  const actual = await importOriginal() as typeof CalendarGridRowModelModule;
  return {
    ...actual,
...
describe("CalendarGrid editor performance", () => {
  it("updates single-day ghost titles without rebuilding span layout", () => {
    ...
    const callsAfterInitialRender = vi.mocked(spanLayoutModule.buildCalendarEventSpanLayout).mock.calls.length;
...
describe("CalendarGrid cell descriptor identity (PERF-01)", () => {
  ...
  function itemStackCallsFor(dateKey: string) {
...
describe("CalendarGrid bills cell descriptor identity (PERF-01 follow-up)", () => {
  ...
  // See the note above `billDayStateCache` in billsModel.ts.
  ...
  const billsActiveView = {
    ...
    getItemId: (bill: CalendarGridItemLike) => bill.scheduleId || bill.id,
  ...
  it("keeps an untouched bills cell's descriptor array referentially identical across an unrelated grid re-render", () => {
    const props = buildGridProps({
      view: "bills",
      activeView: billsActiveView as unknown as CalendarGridActiveViewContract,
    ...
    // (see the comment above billDayStateCache in billsModel.ts). This test
