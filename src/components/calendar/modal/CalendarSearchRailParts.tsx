...
import { calendarSearchPlaceholder } from "../../../hooks/calendar/calendarModalSearchModel";
...
import { formatAgendaHeaderLabel } from "../views/agenda/agendaDateModel.ts";
import { colorWithAlpha } from "../views/events/eventsAgendaColor.ts";
import { WeatherHeader } from "../views/events/EventsAgendaRailParts.tsx";
...
function resultIsPast(result: CalendarSearchResultLike, todayKey: string): boolean {
  return !!result?.itemDate && result.itemDate < todayKey;
}

function resultPrimaryMeta(result: CalendarSearchResultLike): string {
...
function resultDetail(result: CalendarSearchResultLike): string {
  if (isGoogleSpecialDateEvent(result)) return "";
...
function buttonChromeStyle(active = false): CSSProperties {
  return {
    width: 30,
    height: 30,
    borderRadius: 8,
...
function applyButtonHover(event: ReactMouseEvent<HTMLButtonElement>): void {
...
function resetButtonHover(event: ReactMouseEvent<HTMLButtonElement>, active = false): void {
...
export function CalendarSearchRailHeader({
  ...
  search: CalendarModalSearchController;
  stateLabel?: string;
...
export function SearchDateHeader({ dateKey, todayKey, weather, onActivate, registerHeader }: {
  ...
  registerHeader?: (dateKey: string, node: HTMLButtonElement | null) => void;
}) {
