...
import { clampFloatingPosition } from "./calendarFloatingDetailPlacement";
import type { CalendarRectLike } from "./calendarFloatingDetailPlacement";
import type { FloatingDetailManualPosition, FloatingDetailMeasuredSize } from "./calendarFloatingDetailRevealModel";

export function rectFromElement(element: Element | null | undefined): DOMRect | null {
  return element?.isConnected ? element.getBoundingClientRect() : null;
}

interface FloatingDetailDragSession {
  pointerId: number;
  placementKey: string;
  offsetX: number;
  offsetY: number;
...
interface FloatingDetailDragPoint { clientX: number; clientY: number }

export interface FloatingDetailDragOptions {
  placementKey?: string | null;
  measuredSize: FloatingDetailMeasuredSize;
...
export default function useFloatingDetailDrag({
  ...
  panelRef,
  onUserDraggedChange,
}: FloatingDetailDragOptions) {
  const draggingRef = useRef(false);
  const dragSessionRef = useRef<FloatingDetailDragSession | null>(null);
  const dragRafRef = useRef(0);
  const pendingDragPointRef = useRef<FloatingDetailDragPoint | null>(null);
  const [manualPosition, setManualPosition] = useState<FloatingDetailManualPosition | null>(null);
