...
export interface PaymentPresentationRow {
  ...
  utilityId?: string;
  scheduleId?: string;
  status: 'paid' | 'received' | 'transferred' | 'scheduled' | 'nothing_due' | 'unknown';
  ...
  statementHistory: UtilityStatement[];
...
export function paymentPresentation(data: FinanceWorkspace, month: string, history: JournalRange | null | undefined = data.recordedHistory) {
  ...
  for (const owner of owners) {
    ...
    const cycles: Array<{ statements: UtilityStatement[]; occurrence?: ActualBillOccurrence }> = statements.map(group => ({ statements: group }));
    ...
    const targetFor = (date: string | null, statement?: UtilityStatement): FinanceDestination => owner.utilityId
    ...
    const base = { name: owner.name, provider: owner.provider, group: owner.utilityId ? 'utilities' as const : 'recurring' as const,
  ...
  for (const row of rows) {
    for (const payment of row.payments) {
      ...
      payments.push({ id: payment.id, name: row.name, payee: row.provider, date: payment.date, amountCents: payment.amountCents,
        direction: row.direction, status: 'recorded', utilityId: row.utilityId, target: row.target });
    }
    if (row.status === 'scheduled' && row.dueDate?.startsWith(month)) {
      payments.push({ id: row.id, name: row.name, payee: row.provider, date: row.dueDate, amountCents: row.amountCents,
        direction: row.direction, status: 'scheduled', utilityId: row.utilityId, target: row.target });
    }
  }
