...
import { ArrowUpRight,Search,ChevronLeft,ChevronRight,CalendarDays } from 'lucide-react';
...
export default function FinancesWorkspace({search,active,scrollTopRequestId=0}:{search:string;active:boolean;scrollTopRequestId?:number}) {
  ...
  useEffect(()=>{
    ...
    const find=(event:KeyboardEvent)=>{if((event.metaKey||event.ctrlKey)&&event.key.toLowerCase()==='f'&&!event.altKey){event.preventDefault();if(window.matchMedia('(max-width: 767px)').matches)navigate(financesHref({view:'utilities',month:paymentCalendar.month}));requestAnimationFrame(()=>{searchRef.current?.focus();searchRef.current?.select();});}};
  ...
  const [calendarPreview,setCalendarPreview]=useState<{rowId:string;date:string;month:string}|null>(null);
  ...
  const openPayment=(payment:FinancePayment,trigger:HTMLButtonElement)=>{
    const row=rows.find(item=>item.id===payment.id || item.payments.some(record=>record.id===payment.id));
    if(row && payment.date){previewTriggerRef.current=trigger;paymentCalendar.setSelectedDate(payment.date);setCalendarPreview(current=>current?.rowId===row.id&&current.month===paymentCalendar.month?null:{rowId:row.id,date:payment.date!,month:paymentCalendar.month});}
  };
  ...
  return <main ref={workspaceRef} className="fin-workspace" data-view={destination.view}><header className="fin-heading"><h1>Finances</h1><div className="fin-foreground-links"><button className="relative" onClick={()=>navigate(financialHref({view:'all'}))}>Activity<ArrowUpRight size={14} aria-hidden="true"/><FinancialAttentionBadge count={attentionCount} floating/></button></div></header>
    ...
    {data&&(destination.view==='journal'?<FinanceJournal key={`${destination.date || "recent"}:${destination.transactionId || ""}`} date={destination.date} transactionId={destination.transactionId} data={data} revision={revision} onNavigate={go}/>:<>
      <div className="fin-payment-toolbar"><div className="fin-month-control"><button aria-label="Previous payment month" disabled={paymentCalendar.month<=data.start.slice(0,7)} onClick={()=>changeMonth(shiftFinanceMonth(paymentCalendar.month,-1))}><ChevronLeft size={16}/></button><h2><button className="fin-current-month" title="Return to current month" aria-label="Return to current month" onClick={()=>changeMonth(data.end.slice(0,7))}>{new Date(`${paymentCalendar.month}-01T12:00:00`).toLocaleDateString('en-US',{month:'long',year:'numeric'})}</button></h2><button aria-label="Next payment month" disabled={paymentCalendar.month>=paymentCalendar.through.slice(0,7)} onClick={()=>changeMonth(shiftFinanceMonth(paymentCalendar.month,1))}><ChevronRight size={16}/></button></div><label className="fin-search"><Search size={16} aria-hidden="true"/><span className="sr-only">Find utility or recurring payment</span><input ref={searchRef} type="search" placeholder="Find a bill or payment" value={query} onChange={event=>setQuery(event.target.value)}/></label><button className="fin-mobile-calendar-toggle" aria-expanded={showCalendar} onClick={()=>setShowCalendar(value=>!value)}><CalendarDays size={16}/>{showCalendar?'Hide calendar':'Show calendar'}</button></div>
      <div className="fin-grid"><section className="fin-overview" ref={overviewRef} aria-label="Bills and payments">
        ...
        <PaymentStatusList today={data.end} rows={visible} selectedId={selected?.id} selectedDate={paymentCalendar.selectedDate} onPreview={paymentCalendar.setPreviewDate} onSelect={openRow}/>
