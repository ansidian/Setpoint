export type FinanceDestination =
  | { view:'utilities'; utilityId?:string; month?:string; statementId?:string; rowId?:string }
...
export function financeDestination(search:string):FinanceDestination {
  ...
  if (query.get('schedule')) return { view:'schedule', scheduleId:query.get('schedule')!, date, month:validDate(`${query.get('month')}-01`)?query.get('month')!:undefined, rowId:query.get('row') || undefined };
  return { view:'utilities', utilityId:query.get('utility') || undefined,
    month:validDate(`${query.get('month')}-01`) ? query.get('month')! : undefined, statementId:query.get('statement') || undefined, rowId:query.get('row') || undefined };
}
