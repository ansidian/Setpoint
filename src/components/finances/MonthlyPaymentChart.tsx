...
export default function MonthlyPaymentChart({ payments, statements, month, historyComplete, onNavigate }: {
  payments: RecordedPayment[]; statements: UtilityStatement[]; month: string; historyComplete: boolean; onNavigate: (target: FinanceDestination) => void;
}) {
  ...
  const current = amounts.find(value => value.month === (hovered || focused));
  const selected = amounts.find(value => value.month === selectedMonth);
  const max = Math.max(1, ...amounts.map(value => value.amountCents ?? 0));
  const describe = (value: (typeof amounts)[number]) => `${monthLabel(value.month)}: ${value.amountCents === null ? 'No complete payment amount available' : `${financeMoney(value.amountCents)}${!historyComplete ? ' recorded so far' : ''}`}`;
  return <section className="fin-history fin-monthly-chart">
    ...
    <div className="fin-history-content">
      ...
      <div ref={chartRef} className="fin-chart" role="group" aria-label="Amounts paid by recording month">
        {amounts.map(value => <button key={value.month} aria-pressed={selectedMonth === value.month} data-preview={current?.month === value.month} aria-controls={selectedMonth === value.month ? "selected-month-payments" : undefined} aria-haspopup={mobile ? undefined : "dialog"} onClick={event => { anchorRef.current = event.currentTarget; setSelectedMonth(value.month); }} aria-label={describe(value)} onFocus={event => setFocused(event.currentTarget.matches(':focus-visible') ? value.month : null)} onBlur={() => setFocused(null)} onMouseEnter={() => setHovered(value.month)} onMouseLeave={() => setHovered(null)}>
          <span className="fin-bar-space">{value.amountCents !== null ? <i style={{ height: `${Math.max(2, value.amountCents / max * 100)}%` }}/> : <span aria-hidden="true">—</span>}</span>
          <span className="fin-month-label">{monthLabel(value.month).split(' ')[0]}</span>
        </button>)}
      </div>
      {selected && <PaymentMonthDetails key={`${selected.month}:${mobile}`} selected={selected} mobile={mobile} anchorRef={anchorRef} onClose={() => setSelectedMonth(null)} onNavigate={onNavigate}/>}
    </div>
  </section>;
}
