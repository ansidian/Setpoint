...
function JournalDatePicker({date,end,maxDate,onSelect}:{date?:string;end:string;maxDate:string;onSelect:(date:string)=>void}) {
  const [open,setOpen]=useState(false);
  const [nowTick]=useState(()=>Date.now());
  const anchorRef=useRef<HTMLButtonElement>(null);
  const close=()=>{setOpen(false);anchorRef.current?.focus();};
  return <>
    <button ref={anchorRef} className="fin-date-trigger" aria-label={date ? `Go to a day, ${financeDate(date)}` : 'Go to a day'} aria-haspopup="dialog" aria-expanded={open} onClick={()=>setOpen(value=>!value)}><CalendarDays size={15}/><span className="fin-date-trigger-label">{date ? financeDate(date) : 'Go to a day'}</span><span className="fin-date-trigger-compact">Date</span><ChevronDown size={13}/></button>
    {open&&<AnchoredFloatingPanel anchorRef={anchorRef} onClose={close} width={300} height={386} mobileHeight={null} role="dialog" ariaLabel="Journal date" style={{overflow:'hidden',padding:8}}>
      <div className="fin-journal-date-picker"><CalendarDateTimeView maxDate={maxDate} nowTick={nowTick} initialEpoch={new Date(`${date || end}T12:00:00Z`).getTime()} onSelect={epoch=>{close();onSelect(new Intl.DateTimeFormat('en-CA',{timeZone:DASHBOARD_TZ,year:'numeric',month:'2-digit',day:'2-digit'}).format(epoch));}} onBack={close} accent="var(--ea-accent)" confirmLabel="View day" mode="date-only" allowPastDates submitOnDateSelect/></div>
