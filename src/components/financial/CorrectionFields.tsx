...
export default function CorrectionFields({ draft, inspection, disabled, onChange }: {
  ...
  const transfer = draft.type === 'transfer' || draft.type === 'transfer_schedule';
  const scheduledTransfer = draft.type === 'transfer_schedule';
  ...
  return <fieldset disabled={disabled} className="space-y-4">
    <div className="financial-field"><span>Record type</span><Dropdown ariaLabel="Record type" value={draft.type} disabled={disabled} onChange={type => patch({ type:type as FinancialCorrectionDraft['type'] })} options={Object.entries(typeLabels).filter(([id]) => scheduledTransfer ? id === 'transfer_schedule' : id !== 'transfer_schedule').map(([id,name]) => ({ id,name }))} /></div>
    <div className="financial-fields">
      <label className="financial-field"><span className="financial-field-label"><Wallet size={13} aria-hidden="true" />{transfer ? 'Transfer' : draft.type === 'income' ? 'Inflow' : 'Outflow'} amount (USD)</span><input type="number" min="0.01" step="0.01" required value={draft.amountCents ? draft.amountCents / 100 : ''} onChange={event => patch({ amountCents:Math.round(Number(event.target.value) * 100) })} /></label>
      ...
      {(draft.type === 'bill' || scheduledTransfer) && <label className="financial-field">Schedule name<input maxLength={2000} value={draft.name || ''} onChange={event => patch({ name:event.target.value })} /></label>}
    </div>
    <p className="financial-note">Enter a positive amount. {transfer ? 'Money moves from the From account to the To account.' : draft.type === 'income' ? 'This adds money to the selected account.' : 'This takes money out of the selected account; no minus sign needed.'}</p>
    ...
    {transfer && draft.fromAccountId && draft.fromAccountId === draft.toAccountId && <p role="status" className="financial-error">Choose different From and To accounts.</p>}
    {scheduledTransfer && <p className="financial-note">Changes update this scheduled transfer and its next occurrence. Existing recurrence is preserved; no transaction is posted.</p>}
    ...
    {schedule && draft.type !== 'bill' && !scheduledTransfer && <div className="space-y-3">
