...
import { confirmFinancialCorrection, getFinancialCorrection, inspectFinancialCorrection, previewFinancialCorrection } from '../../api';
...
import { initialDraft, intendedDraft, mayRequestSuccessor } from './correctionPresentation';
...
export default function FinancialCorrectionEditor({ activity, onDirty, onChanged, onRepair, registerBack, requestDiscard, onEditing, onHistoryChange }: {
  ...
  useEffect(() => {
    if (correction) onHistoryChange({ id:correction.id, predecessorId:correction.preview.predecessorId, state:correction.state,
      updatedAt:correction.updatedAt, steps:correction.steps.map(({ state,attemptedAt }) => ({ state,attemptedAt })) });
  ...
  useEffect(() => { onEditing(editing || uncertainConfirm); },[editing,uncertainConfirm,onEditing]);
  useEffect(() => {
    const stage = preview ? 'preview' : editing ? 'edit' : 'record';
  ...
  useEffect(() => {
    ...
    registerBack(busy || uncertainConfirm ? () => true : preview ? () => { setPreview(null); return true; } : editing ? () => { discard(); return true; } : null);
    return () => registerBack(null);
  },[preview,editing,busy,uncertainConfirm,registerBack,requestDiscard,onDirty]);
  useEffect(() => {
    ...
    const refresh = async () => {
      try { const result = await getFinancialCorrection(id); if (!active) return; setCorrection(result);
        ...
        if (!['completed','superseded'].includes(result.state)) timer = setTimeout(() => void refresh(),5000);
  ...
  async function prepare() {
    ...
    lock.current = true; setBusy(true); setError(''); setRepairAvailable(false);
    try { const result = await previewFinancialCorrection(activity.reference,intendedDraft(draft)); if (alive.current) { setPreview(result); key.current = crypto.randomUUID(); setUncertainConfirm(false); } }
  ...
  const canEdit = !blocked || successor;
  ...
  const hasAccounts = draft && (draft.type === 'transfer'
  ...
  return <section ref={surface} className="financial-section" aria-label="Correct Actual record">
    <AnimatedHeight><div className="p-1">
      ...
      {correction && !editing && <div className="financial-status-panel" data-tone={correction.state === 'completed' ? 'success' : 'attention'}>
        <div role="status">
          <h3>{correction.state === 'completed' ? <CheckCircle2 size={16} /> : <CircleAlert size={16} />}{correction.state === 'completed' ? 'Correction saved to Actual' : correction.state === 'attention' ? 'Correction needs attention' : 'Checking correction progress'}</h3>
          <p>{correction.state === 'completed' ? 'Your corrected result is shown above. The original receipt is still available below.' : successor ? 'Some changes were saved. Review what remains, then preview the next update.' : 'Setpoint is checking what reached Actual. You can close this record and return later.'}</p>
        </div>
      ...
      {!editing && !uncertainConfirm && canEdit && <div className="financial-actions">
        <button type="button" data-correction-focus className="financial-button financial-primary" disabled={busy} onClick={() => void inspect(true)}><Pencil size={14} />{busy ? 'Reading Actual…' : successor ? 'Review remaining changes' : 'Correct record'}</button>
      </div>}
      ...
      {!preview && !uncertainConfirm && (canEdit || inspection) && <div className="mt-4">
        ...
        <AnimatedCollapse open={showInspection}><div className="financial-disclosure-body">
          {inspection ? <><SnapshotSummary snapshot={inspection.snapshot} /><p className="financial-note">Actual may have changed since this read. Preview checks it again.</p>{!(error && editing && !repairAvailable) && <button type="button" className="financial-button mt-3" disabled={busy} onClick={() => void inspect(false)}><RefreshCw size={14} />{busy ? 'Reading Actual…' : 'Refresh from Actual'}</button>}</> : <p role="status" className="financial-note">{busy ? 'Reading current Actual details…' : 'Current details are unavailable.'}</p>}
        </div></AnimatedCollapse>
      </div>}
    </div></AnimatedHeight>
  </section>;
