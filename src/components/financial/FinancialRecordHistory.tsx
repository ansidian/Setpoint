...
function SourceEmail({ uid }: { uid:string }) {
...
function SavedReceipt({ receipt, metadata }: { receipt:FinancialOriginalReceipt; metadata:ActualMetadata|null }) {
  ...
  return <div className="financial-receipt">
    <div className="financial-receipt-header"><ReceiptText size={16} aria-hidden="true" /><div><strong>{receipt.captureKind === 'historical' ? 'Older saved result' : 'Saved result'}</strong><p>Captured <time dateTime={new Date(receipt.capturedAt).toISOString()}>{new Date(receipt.capturedAt).toLocaleString()}</time></p></div><span className="financial-receipt-outcome">{receipt.outcome.replace(/_/g,' ')}</span></div>
    {objects.length ? <SnapshotSummary snapshot={snapshot} /> : <><dl className="financial-summary"><div><dt>Amount</dt><dd>{money(fields.amountCents)}</dd></div>{Boolean(fields.date) && <div><dt>Date</dt><dd>{dateLabel(fields.date)}</dd></div>}</dl><div className="financial-evidence-note"><Info size={16} aria-hidden="true" /><div><strong>Limited saved details</strong><p>Detailed original Actual fields were not captured.</p></div></div></>}
  ...
  return <section className="financial-section financial-history" aria-label="Record history">
    ...
    {items.length === 0 ? <p className="financial-note">No saved history is available yet.</p> : <ol className="financial-history-list">
      {items.map(item => {
        ...
        const title = item.kind === 'email' ? 'Email received' : item.kind === 'receipt' ? item.receipt.captureKind === 'historical' ? 'Older result captured' : 'Original result saved' : correctionLabels[item.correction.state];
