...
export function timeClock(iso: string | null | undefined): string {
  if (!iso) return "";
...
export function buildSnoozePresets(nowMs: number): SnoozePreset[] {
  return [
...
export function computePlacement(anchorRect: Pick<DOMRect, "top" | "bottom" | "left" | "right">, panelW: number, panelH: number) {
  const margin = 10;
