...
function rowsById(rows: InboxEmailLike[]): Map<string, InboxEmailLike> {
...
describe("reconcileSnapshotOverlays", () => {
  ...
  it("keeps hidden and handled overlays until their row disappears", () => {
    ...
    const prev = new Map<string, SnapshotOptimisticOverlay>([["1", hidden], ["2", handled]]);
    const rows = rowsById([
