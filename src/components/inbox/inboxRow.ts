...
interface BuildInboxRowOptions {
  uid: string;
  id?: InboxId;
...
export function readOverrideForUid(readOverrides: InboxReadOverrides, uid: string | null | undefined): boolean | null {
  if (!uid || !readOverrides) return null;
  if (readOverrides instanceof Map) {
    return readOverrides.has(uid) ? (readOverrides.get(uid) ?? null) : null;
  }
  const record = readOverrides as Readonly<Record<string, boolean | null | undefined>>;
  return Object.prototype.hasOwnProperty.call(readOverrides, uid)
    ? (record[uid] ?? null)
    : null;
}

export function mergeReadState(read: boolean | null | undefined, uid: string | null | undefined, readOverrides: InboxReadOverrides): boolean {
...
export function composeReadOverrides(...sources: InboxReadOverrides[]): Record<string, boolean> {
...
export function buildInboxRow(raw: InboxEmailLike, {
  uid,
  id = raw.id || uid,
  account,
  read = raw.read,
  ...
  resurfacedAt = null,
  extras = null,
}: BuildInboxRowOptions): NormalizedInboxRow {
  return {
    ...raw,
    id,
    uid,
