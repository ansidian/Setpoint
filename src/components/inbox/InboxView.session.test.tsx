...
describe("InboxView session state", () => {
  ...
  it("refreshes active snapshot when a stale handled action is rejected", async () => {
    const refreshSnapshot = vi.fn().mockResolvedValue({});
    vi.mocked(markSnapshotItemHandled).mockRejectedValueOnce(
  ...
  it("moves an active snapshot row to another lane immediately and refreshes afterward", async () => {
    ...
    vi.mocked(moveSnapshotItemLane).mockImplementationOnce(() => new Promise<SnapshotItem>((resolve) => {
      resolveMove = () => resolve({} as SnapshotItem);
    }));
    const refreshSnapshot = vi.fn().mockResolvedValue({});
    const activeSnapshot = {
      snapshot: makeActiveSnapshot({
  ...
  it("hides an active snapshot row immediately when dismissed", async () => {
    vi.mocked(dismissSnapshotItemForToday).mockImplementationOnce(() => new Promise(() => {}));
    const activeSnapshot = {
      snapshot: makeActiveSnapshot({
        lanes: {
          needs_attention: [{
    ...
    vi.mocked(markSnapshotItemHandled).mockImplementationOnce(() => new Promise<SnapshotItem>((resolve) => {
      resolveMutation = () => resolve({} as SnapshotItem);
    }));
    const activeSnapshot = {
      snapshot: makeActiveSnapshot(),
      loading: false,
