...
import type { NewsSourcePreview, NewsSourceKind } from "../../../shared/types/news.ts";

interface NewsAddSourceFormProps {
  topicId: number;
  onAdded?: () => void;
  onCancel?: () => void;
}

export default function NewsAddSourceForm({ topicId, onAdded, onCancel }: NewsAddSourceFormProps) {
  const [mode, setMode] = useState<NewsSourceKind>("rss");
  const [url, setUrl] = useState("");
  const [checking, setChecking] = useState(false);
  const [preview, setPreview] = useState<NewsSourcePreview | null>(null);
  ...
  async function handleCheck() {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Could not check that URL.");
    } finally {
      setChecking(false);
    }
  }
