...
function HeaderButton({ onClick, disabled = false, children, ariaLabel, ariaPressed, className = "" }: HeaderButtonProps) {
  return (
    <button
      type="button"
...
function checkedLabel(lastUpdatedAt: string | null): string {
...
function skeletonBar(width: CSSProperties["width"], height = 10) {
...
export default function NewsView({
  ...
  onOpenManage: (topicId?: number | null) => void;
  onReload: (options?: { background?: boolean }) => void | Promise<void>;
}) {
  const viewRef = useRef<HTMLDivElement>(null);
  const [trackedTopicId, setTrackedTopicId] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    const updateActiveTopic = () => {
      const sections = [...view.querySelectorAll<HTMLElement>("[data-news-topic-id]")];
      if (!sections.length) return;
      const rootTop = scrollRegion?.getBoundingClientRect().top || 0;
      const toolbarHeight = view.querySelector<HTMLElement>(".news-toolbar")?.offsetHeight || 0;
      const activationLine = rootTop + toolbarHeight + 16;
      const sectionTops = sections.map((section) => section.getBoundingClientRect().top);
      let nextTopicId = sections[0]!.dataset.newsTopicId ?? null;

      if (!sectionTops.every((top) => top === sectionTops[0])) {
        sections.forEach((section, index) => {
          if (sectionTops[index]! <= activationLine) nextTopicId = section.dataset.newsTopicId ?? null;
        });
      }
  ...
  if (error && !news) {
    return (
      <EmptyStateSplash
        ...
        actions={<HeaderButton onClick={() => { void onReload(); }} ariaLabel="Retry">Retry</HeaderButton>}
      />
    );
  }
