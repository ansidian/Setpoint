...
describe("NewsView", () => {
  ...
  it("keeps the topic index in sync with the section under the sticky toolbar", () => {
    ...
    const scrollRegion = container.querySelector("[data-scroll-lock-target]");
    ...
    vi.spyOn(scrollRegion, "getBoundingClientRect").mockReturnValue({ top: 0 });
    vi.spyOn(hardwareSection, "getBoundingClientRect").mockReturnValue({ top: -140 });
    vi.spyOn(aiSection, "getBoundingClientRect").mockReturnValue({ top: 10 });
    vi.spyOn(cookingSection, "getBoundingClientRect").mockReturnValue({ top: 320 });
