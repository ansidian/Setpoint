...
export interface NewsSectionPlan<T> {
  hasItems: boolean;
  quiet: boolean;
  nothingNew: boolean;
  freshCount: number;
...
export interface NewsTopicSourceHealth {
  count: number;
  label: string;
  tone: "warning" | "danger";
}

type NewsSourceView = Partial<Pick<NewsSource,
  "enabled" | "lastStatus" | "consecutiveFailures" | "feedUrl" | "siteUrl" | "title"
>>;

export function splitItemsBySeen<T extends { publishedAt: string | null }>(items: T[], dividerAt: string | null) {
...
export function resolveDividerMarker(
  payloadLastSeenAt: string | null,
...
export function displayExcerpt(excerpt: string | null | undefined): string {
  if (!excerpt) return "";
...
export function planTopicSection<T extends { publishedAt: string | null }>(
  items: T[],
  dividerMarker: string | null,
  { hideSeen = false }: { hideSeen?: boolean } = {},
): NewsSectionPlan<T> {
...
export function describeSourceHealth(source: NewsSourceView | null | undefined): NewsSourceHealth {
...
function isRedditSource(source: NewsSourceView | null | undefined): boolean {
...
export function summarizeTopicSourceHealth(sources: NewsSourceView[] = []): NewsTopicSourceHealth | null {
