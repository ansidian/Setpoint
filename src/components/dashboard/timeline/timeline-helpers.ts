...
import type { TimelineDeadline, TimelineEvent } from "../../../lib/shell-helpers";
import { formatReminderSummary } from "../../calendar/reminderDisplay";
...
export interface DashboardTimelineItem {
  ...
  dueAtMs?: number;
  sortKey?: number;
  data?: (TimelineEvent & TimelineDeadline & {
    allDay?: boolean;
    status?: string;
...
export function formatFullDateForOffset(offset: number, now: number) {
...
export function shouldHoldPartialTimeline({
  ...
  filtersDeadlines?: boolean;
  deadlinesLoading?: boolean;
  hasDeadlineRows?: boolean;
}) {
...
export function buildTimelineGroups(
  ...
  { minDay = null }: { minDay?: number | null } = {},
): TimelineGroup[] {
  const groups = new Map<number, DashboardTimelineItem[]>();
 ...
 */
export function percentElapsed(startMs: number, endMs: number, nowMs: number) {
 ...
 */
export function formatNowMarkerClock(nowMs: number) {
 ...
 */
export function formatNowMarkerLabel(nowMs: number, percent: number) {
 ...
 */
export function resolveTodayNowMarkerIndex(items: DashboardTimelineItem[], now: number): number | null {
 ...
 */
export function deriveTimelineRowState(
  ...
  const reminderSummary = formatReminderSummary(item.data, { now: new Date(now) });
 ...
 */
export function buildTodayTomorrowRestGroups(items: DashboardTimelineItem[], now: number, filters: TimelineFilters) {
