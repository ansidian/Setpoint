...
import useDashboardItemSheet from "./useDashboardItemSheet.js";
...
describe("useDashboardItemSheet", () => {
  it("owns selection and closes the dashboard sheet when its tab is left", () => {
    ...
    const { result, rerender } = renderHook(
      ({ tab }) => useDashboardItemSheet({ tab, openCalendar }),
  ...
  ])("closes the %s sheet before handing it off to the calendar", (sheet, expectedCall) => {
    const openCalendar = vi.fn();
    const { result } = renderHook(() => useDashboardItemSheet({
      tab: "dashboard",
      openCalendar,
