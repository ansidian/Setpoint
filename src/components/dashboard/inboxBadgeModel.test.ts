...
import type { ActiveSnapshotInput, DashboardEmail } from "./inboxBadgeModel";

type SnapshotOverrides = Omit<DashboardEmail, "snapshot_item_id"> & {
  snapshot_item_id?: string | number;
  [key: string]: unknown;
};
type SnapshotLanes = Record<string, SnapshotOverrides[]>;

function snapshotRow(overrides: SnapshotOverrides = {}): SnapshotOverrides {
  const uid = String(overrides.uid || overrides.email_id || overrides.id || "msg-1");
  return {
    id: uid,
...
function activeSnapshot(lanes: Partial<SnapshotLanes>): ActiveSnapshotInput {
  return {
    snapshot: { id: 1 },
    filters: {
