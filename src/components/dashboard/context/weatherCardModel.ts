...
function iconColor(name: WeatherIconName) {
...
function condLabel(name: WeatherIconName | undefined) {
  return name ? COND_LABELS[name] || "" : "";
}

function weekdayName(dateKey: string | undefined) {
  if (!dateKey) return "";
...
export function currentCondition(weather?: DashboardWeather | null) {
...
export function buildForecastHours(weather?: DashboardWeather | null) {
...
export function buildForecastDays(weather?: DashboardWeather | null) {
  ...
  return days.map((d) => ({
    ...
    hi: `${d.high}°`,
    lo: `${d.low}°`,
    rain: Number.isFinite(d.rain) && (d.rain ?? 0) >= RAIN_CHIP_THRESHOLD ? `${d.rain}%` : null,
  }));
}
