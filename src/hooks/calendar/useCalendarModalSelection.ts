...
import {
  ...
  type CalendarModalSyncSnapshot,
  parseFocusDate,
  resolveFocusViewDate,
} from "./calendarModalSelectionModel";

export interface CalendarModalSelectionOptions {
  open: boolean;
  view: string;
...
export interface CalendarModalSelectionRuntime {
  open?: boolean;
  view?: string;
  openRequestId?: number;
}
...
export default function useCalendarModalSelection({
  ...
  const commitSyncSnapshot = useCallback((snapshot: CalendarModalSyncSnapshot | null, runtime: CalendarModalSelectionRuntime = {}) => {
    if (snapshot) {
      setViewDate((current) => (
  ...
  const focusDateKey = useCallback((ymd: string) => {
