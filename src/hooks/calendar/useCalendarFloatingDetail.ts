import { useCallback, useEffect, useLayoutEffect, useRef, useState, type MutableRefObject, type RefObject, type SetStateAction } from "react";
import { resolveFloatingDetailPlacement } from "../../components/calendar/modal/calendarFloatingDetailPlacement";
import {
  dateCellSelector,
  type CalendarFloatingDetailState,
  type FloatingDetailSide,
...
export interface CalendarFloatingDetail extends CalendarFloatingDetailState {
  ...
  saveRequestId?: string | number | null;
  activeSaveRequestId?: string | number | null;
  shakeKey?: number;
  initialPlacement?: { caretSide?: string | null } | null;
}

export interface CalendarFloatingDetailOptions {
  open: boolean;
  view: string;
...
type SessionSideRef = MutableRefObject<Record<string, FloatingDetailSide>>;

function elementRect(element: Element | null | undefined) {
  return element?.isConnected ? element.getBoundingClientRect() : null;
}

function rememberedSessionSide(sessionSideByViewRef: SessionSideRef, nextDetail: CalendarFloatingDetail, nextView: string) {
...
function floatingDetailSessionKey(view: string | null | undefined, dateKey: string | null | undefined) {
  return view && dateKey ? `${view}:${dateKey}` : null;
}

function clearSessionSide(sessionSideByViewRef: SessionSideRef, detail: CalendarFloatingDetail | null) {
...
function clearAllSessionSides(sessionSideByViewRef: SessionSideRef) {
  sessionSideByViewRef.current = {};
}

export default function useCalendarFloatingDetail({ open, view, panelRef, railRef }: CalendarFloatingDetailOptions) {
  const [floatingDetail, setFloatingDetail] = useState<CalendarFloatingDetail | null>(null);
  const floatingDetailRef = useRef<CalendarFloatingDetail | null>(null);
  const previousFloatingDetailRef = useRef<CalendarFloatingDetail | null>(null);
  const sessionSideByViewRef = useRef<Record<string, FloatingDetailSide>>({});

  const setSyncedFloatingDetail = useCallback((nextOrUpdater: SetStateAction<CalendarFloatingDetail | null>) => {
  ...
  const findDateCell = useCallback((dateKey: string | null) => {
    if (!dateKey) return null;
    return panelRef.current?.querySelector<HTMLElement>(dateCellSelector(dateKey)) || null;
  }, [panelRef]);
  ...
  const setFloatingEditorDirty = useCallback((dirty: boolean) => {
    setSyncedFloatingDetail((current) => (
  ...
  const setFloatingEditorSaveRequest = useCallback((requestId: string | number | null) => {
    setSyncedFloatingDetail((current) => (
      current?.open && (current.mode === "edit" || current.mode === "create")
  ...
  const openFloatingDetail = useCallback((nextDetail: CalendarFloatingDetail | null) => {
    ...
    setSyncedFloatingDetail((current) => {
      ...
      const initialPlacement = resolveFloatingDetailPlacement({
        ...
        mode,
        preferredSide: resolvedPreferredSide as never,
        forcedSide: forcedSide as never,
  ...
  const setFloatingDetailDragged = useCallback((userDragged: boolean, placementKey: string | null = null) => {
    if (userDragged) {
  ...
  const flipFloatingDetailSide = useCallback(() => {
    ...
    if (!currentDetail || !isGridOriginFloatingDetail(currentDetail) || currentDetail.dirty) return;
    setSyncedFloatingDetail((current) => {
      if (!current || !isGridOriginFloatingDetail(current) || current.dirty) return current;
