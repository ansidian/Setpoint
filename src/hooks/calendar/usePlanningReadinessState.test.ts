...
function planningProps(overrides: Partial<PlanningReadinessStateOptions<{ upcoming: unknown[] }>> = {}): PlanningReadinessStateOptions<{ upcoming: unknown[] }> {
  return {
    open: true,
    view: "events",
    fetchYear: 2026,
    ...
    scrollDirectionRef: { current: "idle" as const },
...
describe("usePlanningReadinessState", () => {
  ...
  it("escalates loading → slow → degraded while deadlines stall", async () => {
    let resolveDeadlines!: (value: { upcoming: unknown[] }) => void;
    const props = planningProps({
      eventsEnsureRange: vi.fn().mockResolvedValue([]),
      deadlinesEnsureRange: vi.fn(() => new Promise<{ upcoming: unknown[] }>((resolve) => { resolveDeadlines = resolve; })),
    });
    const { result } = renderHook(() => usePlanningReadinessState(props));
