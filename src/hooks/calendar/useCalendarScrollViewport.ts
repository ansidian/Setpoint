...
export interface CalendarScrollViewportOptions {
  viewYear: number;
  viewMonth: number;
  currentYear: number;
  currentMonth: number;
  layout: { cellHeight: number; gridGap: number };
  ...
  maybeCancelEditorOnScroll: (programmatic: boolean) => void;
}
...
export default function useCalendarScrollViewport({
  ...
  const alignToWeekRowRef = useRef<(() => void) | null>(null);
    ...
    function onScroll() {
      ...
      rafId = requestAnimationFrame(() => {
        ...
        const scrollArgs = {
          ...
          getMonthOffset: (i: number) => offsets.get(i) ?? 0,
          searchFirst: -SCROLL_RANGE,
          searchLast: SCROLL_RANGE,
        };
