...
export default function useEditorCancelOnScroll({
  ...
  floatingDetailMode: string | null | undefined;
  floatingEditorDirty: boolean;
  onCancelFloatingEditor?: (() => void) | null;
}): (programmaticNavActive: boolean) => void {
  const editorScrollRef = useRef<{ open: boolean; dirty: boolean; cancel?: (() => void) | null }>({
    open: false,
    dirty: false,
    cancel: null,
  });
  ...
  return useCallback((programmaticNavActive: boolean) => {
