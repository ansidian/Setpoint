...
import { monthBounds } from "./calendarRangeModel.js";
import {
  agendaMonthRange,
  initialAgendaMonths,
...
export function useAgendaFetch({
  ...
  const applyLoaded = useCallback((months, { reset = false } = {}) => {
    const set = new Set(reset ? [] : loadedRef.current);
    let changed = reset;
  ...
  const fetchMonths = useCallback(async (months, { reset = false } = {}) => {
    ...
    try {
      await Promise.all([
        domainRangeRef.current.ensureRange(start, end),
        deadlinesRangeRef.current?.ensureRange(start, end),
      ].filter(Boolean));
    } catch {
