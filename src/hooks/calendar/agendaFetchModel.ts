import { monthKey, monthIndex, keyFromMonthIndex } from "./calendarRangeModel";
import type { CalendarDateKey, CalendarMonthKey } from "./calendarRangeModel";
import { NAVIGABLE_MONTH_RADIUS } from "./calendarScrollModel";
...
export interface AgendaMonthRange {
  startKey: CalendarMonthKey;
  endKey: CalendarMonthKey;
  months: CalendarMonthKey[];
}

export type AgendaFetchDirection = "forward" | "backward";

export interface AgendaFetchPlan {
  needed: CalendarMonthKey[];
  direction: AgendaFetchDirection | null;
}

export interface AgendaJumpPlan {
  needed: CalendarMonthKey[];
  reset: boolean;
}

export interface AgendaScrollCommand {
  type: string;
  dateKey?: CalendarDateKey;
  itemId?: string;
}

export function agendaMonthRange(todayKey: CalendarDateKey): AgendaMonthRange {
  const [year, monthStr] = todayKey.split("-").map(Number);
  const todayIdx = monthIndex(monthKey(year ?? 0, (monthStr ?? 1) - 1))!;
...
export function initialAgendaMonths(todayKey: CalendarDateKey): CalendarMonthKey[] {
  const range = agendaMonthRange(todayKey);
  const [year, monthStr] = todayKey.split("-").map(Number);
  const todayMk = monthKey(year ?? 0, (monthStr ?? 1) - 1);
...
export function computeAgendaFetchPlan({
  ...
  topmostMonth: CalendarMonthKey | null | undefined;
  loadedMonths: readonly CalendarMonthKey[];
  totalRange: AgendaMonthRange | null | undefined;
  threshold?: number;
}): AgendaFetchPlan {
  const empty: AgendaFetchPlan = { needed: [], direction: null };
  ...
  let direction: AgendaFetchDirection | null = null;
  const candidates: CalendarMonthKey[] = [];

  if (nearEnd && (!nearStart || fromEnd <= fromStart)) {
    direction = "forward";
    for (let i = loadedLast! + 1; i < allMonths.length && candidates.length < FETCH_BATCH; i++) {
      const candidate = allMonths[i]!;
      if (!loadedSet.has(candidate)) candidates.push(candidate);
    }
  } else {
    direction = "backward";
...
export function scrollCommandTargetMonth(
  scrollCommand: AgendaScrollCommand | null | undefined,
  todayKey: CalendarDateKey | null | undefined,
...
export function pruneAgendaMonths({
  ...
  anchorMonth: CalendarMonthKey | null | undefined;
  loadedMonths?: CalendarMonthKey[];
  keepRadius?: number;
}): CalendarMonthKey[] {
...
export function computeAgendaJumpPlan({
  ...
  targetMonth: CalendarMonthKey | null | undefined;
  loadedMonths?: readonly CalendarMonthKey[];
  totalRange: AgendaMonthRange | null | undefined;
  reach?: number;
  radius?: number;
}): AgendaJumpPlan {
  const empty: AgendaJumpPlan = { needed: [], reset: false };
