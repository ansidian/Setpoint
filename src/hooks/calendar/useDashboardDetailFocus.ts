...
import { dashboardDetailFocusRequest, type DashboardDetailFocusRequest } from "./calendarModalInteractionModel";
import {
  findGridChipAnchor,
  itemDueDate,
...
export interface PendingDashboardDetailFocus extends Omit<DashboardDetailFocusRequest, "detailKind"> {
  ...
  searchResult?: CalendarSearchResultLike | null;
}

export interface DashboardDetailFocusOptions {
  open: boolean;
  view: string;
  ...
  agendaRailRef: MutableRefObject<{ activateItem?: (itemId: string, dateKey: string) => boolean } | null>;
  ...
  pendingItemDetailFocus: PendingDashboardDetailFocus | null;
...
export default function useDashboardDetailFocus({
...
}: DashboardDetailFocusOptions) {
  const handledDashboardDetailFocusRef = useRef<string | null>(null);
  ...
  const attemptDashboardDetailFocus = (request: PendingDashboardDetailFocus) => {
    ...
    if (request.anchorKind === "grid-chip") {
      ...
      openFloatingDetail({
        ...
        sourceCellElement: anchorElement.closest?.("[role='gridcell']") as HTMLElement | null,
        anchorKind: "chip",
        itemsSnapshot: [item],
      });
    ...
    if (request.anchorKind === "search-result-row") {
      ...
      openFloatingDetail({
        ...
        sourceCellElement: request.sourceCellElement?.isConnected
          ? request.sourceCellElement as HTMLElement
