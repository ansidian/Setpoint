...
import type { CalendarMonthPosition } from "./calendarScrollModel";

export interface CalendarScrollSettleInput {
  scrollDataIndex: number;
  scrollMountIndex: number;
  prevView: CalendarMonthPosition | null;
  labelIndex: number;
  refYear: number;
  refMonth: number;
...
export interface CalendarScrollSettleResult {
  shouldDefer: boolean;
  settledMonth?: CalendarMonthPosition;
  settledAway?: boolean;
  scrollDrivenAfter?: boolean;
  displayMonthChange?: CalendarMonthPosition | null;
  labelMonthChange?: CalendarMonthPosition | null;
  fetchSettleArgs?: CalendarMonthPosition & { scrollDriven: boolean };
  shouldAlign?: boolean;
}
...
export function resolveScrollSettle({
  ...
  refMonth,
  wasSuppressed,
}: CalendarScrollSettleInput): CalendarScrollSettleResult {
