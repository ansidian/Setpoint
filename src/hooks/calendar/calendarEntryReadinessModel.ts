...
import type { CalendarDateRange, DeadlineOverlayRecord } from "./calendarControllerHelpers";

export interface CalendarEntryPlanningReadiness {
  state: string;
...
export interface CalendarEntryReadinessInput {
  viewYear: number;
  viewMonth: number;
  eventsLoading: boolean;
  eventsIsMonthLoading?: ((year: number, month: number) => boolean) | null;
  eventsEnsureRange?: ((...args: unknown[]) => unknown) | null;
  deadlineOverlayVisible: boolean;
  committedDeadlineOverlayData: DeadlineOverlayRecord<unknown> | null;
  deadlinesEnsureRange?: ((...args: unknown[]) => unknown) | null;
  deadlinesSeedData: unknown;
  deadlinesDataRange: CalendarDateRange | null;
...
export function computeCalendarEntryReadiness({
  ...
  planningReadiness,
  deadlinesData,
}: CalendarEntryReadinessInput): { eventsRangeLoading: boolean; agendaEntryReady: boolean } {
