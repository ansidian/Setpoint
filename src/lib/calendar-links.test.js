...
describe("calendar-links", () => {
  ...
  it("normalizes bare URLs from event text", () => {
    expect(
      extractUrlsFromText("Canvas link canvas.example.com/courses/123/assignments/456"),
    ).toEqual(["https://canvas.example.com/courses/123/assignments/456"]);
  });
  ...
  it("extracts a bare non-Zoom URL from location", () => {
    expect(
      extractNonZoomEventUrl({
        location: "canvas.example.com/courses/123/assignments/456",
        description: "Backup https://docs.example.com/agenda",
      }),
    ).toBe("https://canvas.example.com/courses/123/assignments/456");
  });
  ...
  it("returns null when only Zoom links are present for the general URL", () => {
    expect(
      extractNonZoomEventUrl({
        location: "example.zoom.us/j/11122233344",
