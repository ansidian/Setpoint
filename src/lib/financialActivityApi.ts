...
export function listFinancialActivity(query: FinancialActivityQuery = {}): Promise<FinancialActivityPage> {
  const params = new URLSearchParams(Object.entries(query).filter(([, value]) => value !== undefined).map(([key, value]) => [key, String(value)]));
  return apiFetch(`/api/briefing/financial-activity?${params}`);
}
