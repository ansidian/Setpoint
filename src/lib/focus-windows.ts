...
interface RawFocusGap {
  startMs: number;
  endMs: number;
  nextEvent: FocusEvent | null;
}

interface ScoredFocusWindow extends RawFocusGap {
  durationMin: number;
  nextInterruptionMin: number;
  startDelayMin: number;
  score: number;
  reachesEndOfDay: boolean;
}

type FocusWindowQuality = "Rest of day open" | "Most protected" | "Cleanest" | "Usable" | "Fragile";
type FocusWindowPurpose = "deep-work block" | "catch-up window" | "admin block";
type FocusWindowContext = "before deadline pressure rises" | "before the day fragments" | "before your next event" | "with the rest of the day open";

export interface DecoratedFocusWindow extends ScoredFocusWindow {
  quality: FocusWindowQuality;
  purpose: FocusWindowPurpose;
...
function pacificDateKey(ms: number): string {
  return PACIFIC_DATE_KEY_FORMATTER.format(new Date(ms));
}

export function endOfPacificDayMs(now: number): number {
...
function summarizePressure(deadlines: readonly FocusDeadline[], now: number): FocusPressure {
  const summary: FocusPressure = {
    overdue: 0,
    today: 0,
    soon: 0,
    level: "low",
...
function relevantPressureDeadlineMs(deadline: FocusDeadline | null | undefined, now: number): number | null {
  if (!deadline || deadline.status === "complete") return null;
  const dueAtMs = dueDateToMs(deadline.due_date, deadline.due_time);
  if (dueAtMs === null || !Number.isFinite(dueAtMs)) return null;
  const bucket = dayBucket(dueAtMs, now);
...
function collectCandidateWindows(events: readonly FocusEvent[], now: number) {
...
function scoreWindow(window: RawFocusGap, pressure: FocusPressure, now: number): ScoredFocusWindow {
...
function chooseQuality(window: ScoredFocusWindow, pressure: FocusPressure, remainingEvents: number): FocusWindowQuality {
...
function choosePurpose(window: ScoredFocusWindow, pressure: FocusPressure): FocusWindowPurpose {
...
function chooseContext(window: ScoredFocusWindow, pressure: FocusPressure, remainingEvents: number): FocusWindowContext {
...
function bestShortGap(rawGaps: readonly RawFocusGap[]): (RawFocusGap & { durationMin: number }) | null {
  return rawGaps
    .map((gap) => ({
      ...gap,
...
export function deriveFocusWindows({
  events = [],
  deadlines = [],
  now = Date.now(),
}: {
...
export function focusPressureTarget(deadlines: FocusDeadline[] = [], now = Date.now()) {
  const relevant = deadlines
    .map((deadline) => ({
      deadline,
      dueAtMs: relevantPressureDeadlineMs(deadline, now),
    }))
    .filter((entry): entry is { deadline: FocusDeadline; dueAtMs: number } => (
      entry.dueAtMs !== null && Number.isFinite(entry.dueAtMs)
    ))
...
export function focusPressureDate(deadlines: FocusDeadline[] = [], now = Date.now()): string | null {
