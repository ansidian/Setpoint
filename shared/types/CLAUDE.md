...
- `dashboard.ts`, `snapshots.ts`, `email.ts`, `calendar.ts`, `tasks.ts`, `reminders.ts`, `notes.ts`, `news.ts` — product surface read/write contracts; `calendar.ts` also owns the serializable event create-seed/source-intent and client-coordination value shapes
