...
function nearestGoogleEventColorId(value) {
...
export function normalizeGoogleEventColorId(value) {
  if (value == null || value === "") return null;
  const colorId = String(value);
  return GOOGLE_EVENT_COLOR_BY_ID[colorId] ? colorId : null;
}

export function googleEventColorForId(value) {
  const colorId = normalizeGoogleEventColorId(value);
  return colorId ? GOOGLE_EVENT_COLOR_BY_ID[colorId] : null;
}

export function googleEventColorIdForSourceHex(value) {
  const hex = normalizeHex(value);
  return GOOGLE_EVENT_COLOR_ID_BY_HEX[hex]
    || GOOGLE_CALENDAR_SOURCE_COLOR_TO_EVENT_COLOR_ID[hex]
    || nearestGoogleEventColorId(hex);
}

export function googleEventColorForSourceHex(value) {
