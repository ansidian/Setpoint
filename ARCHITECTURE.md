...
`server/alfred/alfred-run.ts` owns the provider-neutral tool loop. Domain tools remain read-only. `propose_calendar_event` is a non-mutating exception that validates explicit trusted-owner intent, stages at most one run-local proposal, and commits it to the ephemeral conversation only immediately before a successful `run_end`; failed runs preserve the prior proposal. Anthropic uses the Messages adapter and transient cache breakpoints. OpenAI uses the Responses adapter with SSE, function tools, `store: false`, and full returned output/reasoning-item replay for stateless tool continuation. Both adapters normalize text, tool calls, proposal events, stop state, and token usage into the existing Alfred SSE and analytics contracts.
