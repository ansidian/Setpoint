...
- Owner completion supplies missing facts explicitly and can run while email AI is paused. It preserves original source evidence, invalidates stale automation claims and closes the collection window at confirmation time while retaining capture completeness and immutable Actual admission/recovery. Fresh revisions permit corrections only before admission while waiting or needing review; an attempted entry cannot be resubmitted. Later related receipts use a separate owner-confirmed identity projection and must have compatible account evidence. Source changes remain visible across restarts and authentication refreshes.
