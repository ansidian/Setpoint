...
import { refreshFinancialProviderAssessments, FINANCIAL_PROFILE_REVISION, PROVIDER_EVENT_ELIGIBLE, PROVIDER_DOCUMENT_ELIGIBLE, PROVIDER_ADMISSION_ELIGIBLE } from "./financial-provider-policy.ts";
...
const ORIGINAL_UNGUARDED = `NOT EXISTS (SELECT 1 FROM ea_financial_corrected_sources g WHERE g.user_id=ea_financial_events.user_id AND g.owner='event' AND g.record_id=ea_financial_events.id)`;
...
export interface FinancialDocument {
  ...
  providerAssessment?: FinancialProviderAssessment | null;
...
export interface FinancialEvent {
  ...
  progress?: NonNullable<FinancialEmailPlan["workflow"]>["progress"];
}
...
const DOCUMENT_SELECT = `SELECT d.*, e.subject, e.body_text, e.from_name, e.from_address,
  ...
  (SELECT c.effective_result_json FROM ea_financial_effective_corrections c
   ...
   WHERE o.user_id=d.user_id AND o.owner='event' AND o.record_id=d.event_id) AS correction_result_json,
...
const PROFILE_CHANGED = `attempted_at IS NULL AND owner_completion_json IS NULL AND plan_json IS NOT NULL AND EXISTS (
  SELECT 1 FROM ea_settings settings WHERE settings.user_id = ea_financial_events.user_id
    AND (${FINANCIAL_PROFILE_REVISION} <> COALESCE(json_extract(plan_json, '$.profile.revision'), -1)
...
const READY_EVENT = `dismissed_at IS NULL AND ${ACTIVE_EVENT} AND (next_attempt_at IS NULL OR next_attempt_at <= ? OR (${PROFILE_CHANGED}))
  AND ${PROVIDER_EVENT_ELIGIBLE} AND (attempted_at IS NOT NULL OR ${DOCUMENTS_READY})`;
...
export function documentFromRow(row: Row): FinancialDocument {
  ...
  return projectFinancialDocumentSource({
    ...
    processingPolicy: row.processing_policy === "provider_v1" ? "provider_v1" : "legacy",
    providerAssessment: readJson<FinancialProviderAssessment>(row.provider_assessment_json),
    contentHash: nullableString(row.content_hash), candidate: readJson<BillCandidate>(row.candidate_json),
    correction: correction && { ...correction, ...(kept && correction.state === 'completed' ? { resolution: 'kept_actual' as const } : {}) },
    correctedEntry: result?.entry || null,
...
export function eventFromRow(row: Row, documents: FinancialDocument[]): FinancialEvent {
  return {
    ...
    dismissedAt: nullableNumber(row.dismissed_at), status: String(row.status) as EventStatus, attempts: Number(row.attempts),
...
export function createFinancialEventStore(dbClient: StoreDb = db, now = Date.now) {
    ...
    const claimed = await dbClient.execute({
      sql: `UPDATE ea_financial_documents SET status = 'processing', claim_token = ?, claimed_at = ?,
            ...
            WHERE id = (SELECT next.id FROM ea_financial_documents next
              WHERE next.dismissed_at IS NULL AND ${PROVIDER_DOCUMENT_ELIGIBLE} AND (next.status = 'pending' OR (next.status = 'retry' AND next.next_attempt_at IS NOT NULL)) AND (next.next_attempt_at IS NULL OR next.next_attempt_at <= ?)
  ...
  async function settleDocument(claim: FinancialDocument, input: {
    ...
    assessment?: FinancialProviderAssessment;
    nextAttemptAt?: number | null;
    error?: string | null;
  }): Promise<boolean> {
    const timestamp = now();
    const results = await dbClient.batch([
      {
        sql: `UPDATE ea_financial_events SET revision = revision + 1,
                ...
                next_attempt_at = NULL, updated_at = ?
              WHERE user_id = ? AND (? = 'ignored' OR ? = 'review') AND attempted_at IS NULL AND owner_completion_json IS NULL AND id = (
                SELECT event_id FROM ea_financial_documents WHERE user_id = ? AND id = ?
                  AND claim_token = ? AND revision = ? AND status = 'processing')`,
        args: [timestamp, claim.userId, input.status, input.assessment?.status ?? null, claim.userId, claim.id, claim.claimToken, claim.revision],
      },
      {
        sql: `UPDATE ea_financial_documents SET status = ?, candidate_json = ?, content_hash = ?,
              provider_assessment_json = COALESCE(?, provider_assessment_json),
              processed_revision = CASE WHEN ? = 'ignored' OR ? = 'review' THEN revision ELSE processed_revision END,
        ...
        args: [input.status, input.status === "ignored" ? null : input.candidate ? writeJson(input.candidate) : null, input.contentHash,
          input.assessment ? writeJson(input.assessment) : null, input.status, input.assessment?.status ?? null, input.nextAttemptAt ?? null, input.error ?? null, timestamp,
  ...
  async function saveProviderAssessment(claim: FinancialDocument, assessment: FinancialProviderAssessment): Promise<boolean> {
    const result = await dbClient.execute({sql: `UPDATE ea_financial_documents SET provider_assessment_json=?
      WHERE user_id=? AND id=? AND claim_token=? AND revision=? AND status='processing'`,
      args: [writeJson(assessment), claim.userId, claim.id, claim.claimToken, claim.revision]});
  ...
  async function associateDocument(claim: FinancialDocument, input: {
    ...
    const results = await dbClient.batch([
      {
        sql: `INSERT INTO ea_financial_events (id, user_id, created_at, updated_at, next_attempt_at, collection_deadline, collection_required)
              SELECT ?, ?, ?, ?, ?, ?, ? WHERE EXISTS (
              ...
              ON CONFLICT(id) DO UPDATE SET revision = revision + 1,
                ...
                next_attempt_at = CASE WHEN owner_completion_json IS NOT NULL OR attempted_at IS NOT NULL OR MIN(collection_required, excluded.collection_required) = 0
                  THEN excluded.updated_at ELSE excluded.next_attempt_at END,
                updated_at = excluded.updated_at
              WHERE ea_financial_events.user_id = excluded.user_id`,
        args: [input.eventId, claim.userId, timestamp, timestamp, input.nextAttemptAt ?? null, input.nextAttemptAt ?? null, input.collectEvidence === false ? 0 : 1,
      ...
      },
      {
        sql: `UPDATE ea_financial_documents SET status = 'associated', candidate_json = ?, content_hash = ?,
                dismissed_at = (SELECT dismissed_at FROM ea_financial_events WHERE user_id = ea_financial_documents.user_id AND id = ?),
  ...
  async function completeEvent(document: FinancialDocument, event: FinancialEvent | null, input: {
    ...
    // Explicit review closes speculative collection. Only current linked source
    // revisions can hold this entry; unrelated email capture is independent.
    ...
    const results = await dbClient.batch([
      { sql: `INSERT INTO ea_financial_events (id, user_id, created_at, updated_at)
              SELECT ?, ?, ?, ? WHERE ? IS NULL AND EXISTS (
                SELECT 1 FROM ea_financial_documents WHERE user_id = ? AND id = ? AND revision = ? AND dismissed_at IS NULL AND event_id IS NULL)
      ...
      { sql: `UPDATE ea_financial_events SET revision = revision + 1, status = 'pending', plan_json = ?,
                ...
                collection_deadline = ?, next_attempt_at = ?, attempts = 0,
                claim_token = NULL, claimed_at = NULL, updated_at = ?
              WHERE user_id = ? AND id = ? AND revision = ? AND dismissed_at IS NULL AND attempted_at IS NULL AND operation_json IS NULL AND ${ORIGINAL_UNGUARDED}
                ...
                AND EXISTS (SELECT 1 FROM ea_financial_documents WHERE user_id = ? AND id = ? AND revision = ? AND dismissed_at IS NULL AND event_id IS ?)
                AND NOT EXISTS (SELECT 1 FROM ea_financial_event_references WHERE user_id = ? AND reference_key = ? AND event_id <> ?)`,
  ...
  async function dismissCandidate(document: FinancialDocument, event: FinancialEvent | null, input: {
    ...
    const results = await dbClient.batch([
      // Give a dismissed standalone source a durable identity too. The same
      // source/reference guards precede creation, so a stale request leaves no orphan.
      { sql: `INSERT INTO ea_financial_events (id, user_id, created_at, updated_at)
              SELECT ?, ?, ?, ? WHERE ? IS NULL AND EXISTS (
                SELECT 1 FROM ea_financial_documents WHERE user_id = ? AND id = ? AND revision = ?
                  AND dismissed_at IS NULL AND event_id IS NULL AND (candidate_json IS NOT NULL OR json_extract(provider_assessment_json,'$.status')='review'))
                AND NOT EXISTS (SELECT 1 FROM ea_financial_event_references WHERE user_id = ? AND reference_key = ?)`,
        args: [input.eventId, document.userId, timestamp, timestamp, event?.id ?? null,
          document.userId, document.id, document.revision, document.userId, input.referenceKey] },
      { sql: `UPDATE ea_financial_events SET dismissed_at = ?, revision = revision + 1, status = 'settled',
                reason = 'Candidate dismissed by owner.', next_attempt_at = NULL, claim_token = NULL, claimed_at = NULL, updated_at = ?
              WHERE user_id = ? AND id = ? AND revision = ? AND dismissed_at IS NULL
                AND attempted_at IS NULL AND operation_json IS NULL AND owner_completion_json IS NULL AND ${ORIGINAL_UNGUARDED}
                AND COALESCE(json_extract(outcome_json, '$.outcome'), '') NOT IN ('added', 'updated', 'already_present')
                AND COALESCE(json_extract(plan_json, '$.reconciliation.status'), '') NOT IN ('already_recorded', 'already_scheduled')
                AND EXISTS (SELECT 1 FROM ea_financial_documents WHERE user_id = ? AND id = ? AND revision = ? AND event_id IS ?)`,
        args: [timestamp, timestamp, document.userId, input.eventId, revision,
          document.userId, document.id, document.revision, event?.id ?? null] },
      { sql: `UPDATE ea_financial_documents SET dismissed_at = ?, event_id = ?, revision = revision + 1, processed_revision = revision + 1,
                status = 'ignored', next_attempt_at = NULL, last_error = 'Candidate dismissed by owner.',
                claim_token = NULL, claimed_at = NULL, updated_at = ?
              WHERE user_id = ? AND dismissed_at IS NULL AND (event_id = ? OR (id = ? AND revision = ? AND event_id IS NULL))
                AND EXISTS (SELECT 1 FROM ea_financial_events WHERE user_id = ? AND id = ? AND dismissed_at = ? AND revision = ?)`,
        args: [timestamp, input.eventId, timestamp, document.userId, input.eventId, document.id, document.revision,
          document.userId, input.eventId, timestamp, revision + 1] },
      { sql: `INSERT INTO ea_financial_event_references (user_id, reference_key, event_id)
              SELECT user_id, ?, id FROM ea_financial_events WHERE user_id = ? AND id = ? AND dismissed_at = ? AND revision = ?
                AND ? IS NULL AND ? IS NOT NULL`,
        args: [input.referenceKey, document.userId, input.eventId, timestamp, revision + 1, event?.id ?? null, input.referenceKey] },
    ], "write");
    return results[1]!.rowsAffected === 1;
  }
      ...
      { sql: `INSERT INTO ea_financial_event_references (user_id, reference_key, event_id)
              ...
              WHERE user_id = ? AND id = ? AND revision = ? AND claim_token = ? AND status = 'processing'
                AND ? IS NOT NULL AND EXISTS (SELECT 1 FROM ea_financial_events WHERE user_id = ?
                  AND id = ea_financial_documents.event_id AND (owner_completion_json IS NOT NULL OR attempted_at IS NOT NULL))
              ON CONFLICT(user_id, reference_key) DO NOTHING`,
      ...
      sql: `UPDATE ea_financial_documents SET status = 'associated', processed_revision = revision,
            ...
            WHERE user_id = ? AND id = ? AND revision = ? AND claim_token = ? AND status = 'processing'
              AND EXISTS (SELECT 1 FROM ea_financial_events WHERE user_id = ? AND id = ea_financial_documents.event_id
                AND (owner_completion_json IS NOT NULL OR attempted_at IS NOT NULL))`,
    ...
    const result = await dbClient.execute({
      sql: `UPDATE ea_financial_events SET status = 'processing', claim_token = ?, claimed_at = ?,
            ...
            WHERE id = (SELECT id FROM ea_financial_events WHERE ${READY_EVENT} AND ${ORIGINAL_UNGUARDED}
              ORDER BY owner_completion_json IS NOT NULL DESC, attempted_at IS NOT NULL DESC, created_at, id LIMIT 1) AND ${ACTIVE_EVENT} RETURNING *`,
      args: [claimToken, timestamp, timestamp, timestamp],
    });
  ...
  async function saveEvent(claim: FinancialEvent, input: {
    ...
    const result = await dbClient.execute({
      sql: `UPDATE ea_financial_events SET status = ?, plan_json = ?, next_attempt_at = ?, reason = ?,
              ...
              claim_token = NULL, claimed_at = NULL, updated_at = ?
            WHERE user_id = ? AND id = ? AND claim_token = ? AND revision = ? AND status = 'processing' AND ${ORIGINAL_UNGUARDED}`,
  ...
  async function admitOperation(claim: FinancialEvent, operation: unknown, plan?: FinancialEmailPlan): Promise<boolean> {
    ...
    const timestamp = now();
    const result = await dbClient.execute({
      sql: `UPDATE ea_financial_events SET operation_json = ?, attempted_at = ?, updated_at = ?, attempts = 1,
            ...
            WHERE user_id = ? AND id = ? AND claim_token = ? AND revision = ? AND status = 'processing'
              ...
              AND (owner_completion_json IS NOT NULL OR (? = 'matched' AND EXISTS (
                SELECT 1 FROM ea_settings settings WHERE settings.user_id = ea_financial_events.user_id
                  AND settings.actual_budget_sync_id = ? AND ${FINANCIAL_PROFILE_REVISION} = ?)))
              AND ${DOCUMENTS_READY} AND ${PROVIDER_ADMISSION_ELIGIBLE}`,
  ...
  async function recoverStaleClaims(staleBefore = now() - CLAIM_LEASE_MS): Promise<{ documents: number; events: number }> {
    await refreshFinancialProviderAssessments(dbClient);
    const timestamp = now();
  ...
  async function getNextWakeAt({ eventsOnly = false }: { eventsOnly?: boolean } = {}): Promise<number | null> {
    const timestamp = now();
    const result = await dbClient.execute({
      sql: `SELECT MIN(wake_at) AS wake_at FROM (
              SELECT MIN(COALESCE(next.next_attempt_at, ?)) AS wake_at FROM ea_financial_documents next
                WHERE ? = 0 AND next.dismissed_at IS NULL AND ${PROVIDER_DOCUMENT_ELIGIBLE} AND (next.status = 'pending' OR (next.status = 'retry' AND next.next_attempt_at IS NOT NULL)) AND NOT EXISTS (
              ...
              UNION ALL SELECT MIN(COALESCE(next_attempt_at, ?)) FROM ea_financial_events
                WHERE dismissed_at IS NULL AND ${ACTIVE_EVENT} AND ${PROVIDER_EVENT_ELIGIBLE} AND (attempted_at IS NOT NULL OR ${DOCUMENTS_READY}))`,
  ...
  async function getEventById(userId: string, id: string): Promise<FinancialEvent | null> {
    const result = await dbClient.execute({sql: `${FINANCIAL_EVENT_STATUS_SELECT} WHERE user_id=? AND id=?`, args: [userId, id]});
    return result.rows[0] ? projectEvent(result.rows[0]) : null;
  }

  async function getEventForEmail(userId: string, uid: string): Promise<FinancialEvent | null> {
    const result = await dbClient.execute({
      sql: `${FINANCIAL_EVENT_STATUS_SELECT} WHERE user_id = ? AND id = (
        SELECT event_id FROM ea_financial_documents WHERE user_id = ? AND email_uid = ?)`,
      args: [userId, userId, uid],
    });
    return result.rows[0] ? projectEvent(result.rows[0]) : null;
  }
