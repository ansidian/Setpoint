...
- Domain background-worker modules such as `server/email/email-backfill-worker.js` and `server/reminders/reminder-scheduler.js` — the individual stop functions `index.js` passes into `shutdown.ts`'s `stopFns`
