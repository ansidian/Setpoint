...
export interface BillsMirrorDb {
...
// predicate now live in billsMirrorModel.ts; re-export the public ones so
// bills-service.ts's facade pass-through stays intact.
...
export async function loadActualBudgetUrl(userId: string, { dbClient = db }: { dbClient?: BillsMirrorDb } = {}): Promise<string | null> {
  ...
  const value = result.rows?.[0]?.actual_budget_url;
...
async function priorMirrorHasRows(userId: string, { dbClient = db }: { dbClient?: BillsMirrorDb } = {}): Promise<boolean> {
  const result = await dbClient.execute({
...
function clearBillsMirrorTimer(userId: string): void {
...
export async function getBillsMirrorState(userId: string, { dbClient = db }: { dbClient?: BillsMirrorDb } = {}): Promise<{ row: Record<string, unknown> | null; syncHealth: BillsMirrorHealth; actualBudgetUrl: string | null }> {
  const result = await dbClient.execute({
  ...
  return {
    ...
    actualBudgetUrl: typeof row?.actual_budget_url === "string" ? row.actual_budget_url : null,
  };
}

export async function readBillsMirrorRange(userId: string, { start, end }: ActualDateRange, { dbClient = db }: { dbClient?: BillsMirrorDb } = {}) {
...
export async function readBillsMirrorCurrent(userId: string, {
  dbClient = db,
  now = new Date(),
}: DbOptions = {}): Promise<BillsMirrorPayload> {
...
export async function scheduleBillsMirrorRefresh(userId: string, {
  delayMs = 0,
  dbClient = db,
  now = new Date(),
}: DbOptions & { delayMs?: number } = {}): Promise<{ pendingRefreshAt: string }> {
  ...
  const pendingValue = existing.rows?.[0]?.pending_refresh_at;
  const existingPending = typeof pendingValue === "string" ? pendingValue : null;
...
export async function consumeDueBillsMirrorRefresh(userId: string, {
  dbClient = db,
  now = new Date(),
}: DbOptions = {}): Promise<boolean> {
...
export async function clearPendingBillsMirrorRefresh(userId: string, {
  dbClient = db,
  now = new Date(),
}: DbOptions = {}): Promise<void> {
  await dbClient.execute({
    sql: `UPDATE ea_bills_mirror_state
          SET pending_refresh_at = NULL,
              updated_at = ?
...
export async function runDueBillsMirrorRefresh(userId: string, {
  dbClient = db,
  now = new Date(),
}: DbOptions = {}): Promise<{ refreshed: false } | { refreshed: true; payload: BillsMirrorPayload }> {
...
async function refreshBillsMirrorInner(userId: string, {
  actualBudgetUrl = null,
  dbClient = db,
  now = new Date(),
  refreshLocalActual = false,
  ...
  try {
    ...
    const pruneQuery = (table: string, idColumn: string, ids: string[]): InStatement => (ids.length
      ? {
    ...
    const pruneOccurrencesQuery = (ids: string[]): InStatement => (ids.length
      ? {
    ...
    await dbClient.execute({
      sql: `INSERT INTO ea_bills_mirror_state
            ...
            ON CONFLICT(user_id) DO UPDATE SET
              ...
              refresh_started_at = NULL,
              updated_at = excluded.updated_at`,
      args: [userId, actualBudgetUrl, timestamp, errorMessage(err).slice(0, 500), timestamp],
    });
