...
- Preserve source rows and table relationships when associating labels with values. If several labels precede several values, use explicit structural or repeated source evidence; do not guess from proximity. Do not label an informational zero balance as statement_balance without explicit statement-balance evidence. Preserve conflicting statement amounts when the source cannot resolve them.
