...
describe("currentPayloadFromOccurrences", () => {
  it("splits a 7-day bills window from the broader -30/+90 allSchedules window", () => {
    const occurrenceAt = (next_date: string): ActualBillOccurrence => ({
      id: next_date,
      scheduleId: next_date,
      name: "Bill",
      payee: "Payee",
      amount: 1,
      type: "bill",
    ...
    const occurrences = [
      occurrenceAt("2026-05-22"), // within 7 days -> bills + allSchedules
      occurrenceAt("2026-06-15"), // beyond 7 days, within +90 -> allSchedules only
      occurrenceAt("2026-04-25"), // before today, within -30 -> allSchedules only
      occurrenceAt("2026-09-01"), // beyond +90 -> neither
    ];
