...
} from "../actual/scheduleMatchModel.js";
...
function scheduleIdentityMatches(schedule, bill, metadata) {
  ...
  const scheduleNames = [schedule.name, metadata.payeeMap?.[payeeId]]
...
function statementAmountMatches(schedule, occurrence, amountCents) {
...
function occurrenceForSchedule(occurrences, schedule, dueDate = null) {
  return occurrences.find((item) => (
    item.scheduleId === schedule.id && (!dueDate || item.next_date === dueDate)
  )) || null;
}

function scheduleEvidence(schedule, occurrence, bill, conflicts = []) {
  return {
    kind: "schedule",
    scheduleId: schedule.id,
    name: occurrence?.name || schedule.name || bill.payee,
    dueDate: occurrence?.next_date || schedule.next_date,
...
function transactionIdentityMatches(transaction, bill) {
  ...
  if (exactMatches.length > 1) {
    return {
      ...
      evidence: {
        ...
        scheduleIds: exactMatches.map((schedule) => schedule.id),
        conflicts: ["identity"],
      },
    };
  }
