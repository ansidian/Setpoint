ALTER TABLE ea_transaction_import_items RENAME TO ea_transaction_import_items_legacy;
...
CREATE TABLE ea_transaction_import_items (
  ...
  CHECK (automatic_safe = 0 OR (
    source <> 'generic' AND external_id IS NOT NULL AND imported_id IS NOT NULL
...
SELECT
  ...
  dismissed_at, created_at, updated_at, email_subject,
  financial_email_plan_json, financial_plan_shadow_json
FROM ea_transaction_import_items_legacy;

DROP TABLE ea_transaction_import_items_legacy;
