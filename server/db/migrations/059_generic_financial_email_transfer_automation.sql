ALTER TABLE ea_transaction_import_items RENAME TO ea_transaction_import_items_before_transfer_automation;
...
CREATE TABLE ea_transaction_import_items (
  ...
  CHECK (automatic_safe = 0 OR (
    ...
    AND transaction_date IS NOT NULL AND currency = 'USD' AND payee IS NOT NULL
    AND (
      (source <> 'generic' AND amount_cents < 0)
      OR (
        source = 'generic' AND actual_account_id IS NOT NULL AND financial_email_plan_json IS NOT NULL
        AND (
          (json_extract(financial_email_plan_json, '$.automation.operationClass') = 'one_time_expense' AND amount_cents < 0)
          OR (json_extract(financial_email_plan_json, '$.automation.operationClass') = 'income' AND amount_cents > 0)
          OR (json_extract(financial_email_plan_json, '$.automation.operationClass') = 'transfer_schedule' AND amount_cents < 0)
        )
      )
    )
  )),
...
SELECT
  ...
  dismissed_at, created_at, updated_at, email_subject,
  financial_email_plan_json, financial_plan_shadow_json
FROM ea_transaction_import_items_before_transfer_automation;

DROP TABLE ea_transaction_import_items_before_transfer_automation;

UPDATE ea_transaction_import_items
SET status = 'queued', attempts = 0, next_attempt_at = NULL, last_error = NULL
WHERE source = 'generic'
  AND status IN ('queued', 'needs_review')
  AND automatic_safe = 0
  AND last_error LIKE 'Transfer check failed:%CHECK constraint failed: automatic_safe = 0 OR%'
  AND json_valid(financial_email_plan_json)
  AND json_extract(financial_email_plan_json, '$.automation.operationClass') = 'transfer_schedule'
