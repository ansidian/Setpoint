...
interface EmailSearchDbRow extends EmailSearchRankingRow {
  uid: string;
  account_id?: unknown;
...
function buildPlanFilters(plan: EmailSearchRetrievalPlan | null | undefined, readFilter: 0 | 1 | null, alias = "idx"): QueryFilters {
...
async function loadLexicalRows(dbClient: Client, userId: string, { textQueries = [], fallbackQuery = "", readFilter, plan, limit, now = Date.now() }: { textQueries?: unknown[]; fallbackQuery?: unknown; readFilter: 0 | 1 | null; plan: EmailSearchRetrievalPlan | null; limit: number; now?: string | number }): Promise<RankedEmailSearchRow<EmailSearchDbRow>[]> {
...
async function loadRowsByUid(dbClient: Client, userId: string, uids: string[], { readFilter, plan }: { readFilter: 0 | 1 | null; plan: EmailSearchRetrievalPlan | null }): Promise<EmailSearchDbRow[]> {
...
export async function retrieveInboxAiSearch(userId: string, {
  ...
  const vectorRows = await loadRowsByUid(dbClient, userId, vector.matches.map((match) => String(match.uid)), { readFilter, plan: effectivePlan });
