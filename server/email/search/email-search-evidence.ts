...
interface EmailSearchEvidencePlan extends Record<string, unknown> {
  semantic_query?: unknown;
  lexical_queries?: unknown;
}

interface EmailSearchEvidenceCandidate extends Record<string, unknown> {
  subject?: unknown;
  from?: { name?: unknown; address?: unknown };
  body_snippet?: unknown;
  scores?: { vector?: unknown; lexical?: unknown };
  provenance?: { lexical?: unknown; lexical_fallback?: unknown };
}

interface EmailSearchEvidenceOptions {
  q?: unknown;
  plan?: EmailSearchEvidencePlan | null;
}
...
function fieldText(candidate: EmailSearchEvidenceCandidate = {}): string {
  return clean([
    candidate.subject,
    candidate.from?.name,
...
function tokensFromText(value: unknown): string[] {
  const tokens: string[] = [];
...
function queryTerms({ q, plan }: EmailSearchEvidenceOptions = {}): string[] {
  const text = [
    q,
    plan?.semantic_query,
...
function plannedPhrases(plan: EmailSearchEvidencePlan | null = {}): string[] {
  return [...new Set(
    (Array.isArray(plan?.lexical_queries) ? plan.lexical_queries : [])
      .map(clean)
...
function hasPlannedPhraseEvidence(candidate: EmailSearchEvidenceCandidate, phrases: string[]): boolean {
...
function fieldTokenMatchCount(candidate: EmailSearchEvidenceCandidate, terms: string[]): number {
...
export function filterEmailSearchCandidatesForEvidence<T extends EmailSearchEvidenceCandidate>(
  candidates: T[] | null | undefined,
  { q, plan }: EmailSearchEvidenceOptions = {},
): T[] {
