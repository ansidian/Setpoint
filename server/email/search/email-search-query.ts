...
export interface EmailSearchInputError extends Error {
  status: number;
  code: string;
}

export function pluralVariants(token: unknown): string[] {
...
export function dateTokenVariants(token: unknown): string[] {
  ...
  const unpad = (n: string) => String(Number(n));
...
export function buildFtsFallbackQueries(raw: unknown): string[] {
  const tokens = String(raw || "")
    .toLowerCase()
    .split(/[^a-z0-9]+/)
...
export function unsupportedSearchFlagError(flag: string): EmailSearchInputError {
  const err = new Error(`Unsupported email search flag: ${flag}`) as EmailSearchInputError;
  err.status = 400;
  err.code = "unsupported_email_search_flag";
  return err;
}

export function invalidSearchFlagError(message: string): EmailSearchInputError {
  const err = new Error(message) as EmailSearchInputError;
  err.status = 400;
  err.code = "invalid_email_search_flags";
  return err;
}

export function parseEmailSearchQuery(raw: unknown): EmailSearchQuery {
  const tokens = String(raw || "").trim().split(/\s+/).filter(Boolean);
  const textTokens: string[] = [];
  let readFilter: 0 | 1 | null = null;

  for (const token of tokens) {
    const normalized = token.toLowerCase();
    if (normalized === "is:read" || normalized === "is:unread") {
      const nextReadFilter: 0 | 1 = normalized === "is:read" ? 1 : 0;
