export const EMAIL_AI_MODEL_CATALOG = [
  {
    provider: "anthropic",
    label: "Anthropic",
...
export const DEFAULT_EMAIL_AI_PROVIDER = "anthropic";
export const DEFAULT_EMAIL_AI_MODEL = "claude-sonnet-4-6";

export function inferEmailAiProviderFromModel(model) {
...
export function getDefaultEmailAiModel(provider = DEFAULT_EMAIL_AI_PROVIDER) {
  return EMAIL_AI_MODEL_CATALOG.find((entry) => entry.provider === provider)?.defaultModel || DEFAULT_EMAIL_AI_MODEL;
}

export function isAllowedEmailAiModel(provider, model) {
