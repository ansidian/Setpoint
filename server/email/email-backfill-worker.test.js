...
describe("processNextBackfillWindow", () => {
  ...
  it("finishes as completed_empty when the backfill reaches its target having indexed nothing", async () => {
    await seedEmailAccount(testState.db.current, { id: "gmail-work", type: "gmail" });
    await seedBackfillState({
      cursor_json: JSON.stringify({ nextWindowEnd: "2025-05-05T00:00:00.000Z" }),
      indexed_count: 0,
    });
    gmailApi.fetchEmailsInRange.mockResolvedValueOnce({
      emails: [],
    ...
    expect(result).toMatchObject({ processed: true, status: "completed_empty", indexed: 0 });
    const state = await readBackfillState();
    expect(state).toMatchObject({
  ...
  it("finishes as completed when the final window is empty but earlier windows indexed mail", async () => {
    await seedEmailAccount(testState.db.current, { id: "gmail-work", type: "gmail" });
    await seedBackfillState({
      cursor_json: JSON.stringify({ nextWindowEnd: "2025-05-05T00:00:00.000Z" }),
      indexed_count: 42,
  ...
  it("finishes as completed_empty on the cursor-at-target path when nothing was ever indexed", async () => {
    await seedEmailAccount(testState.db.current, { id: "gmail-work", type: "gmail" });
    await seedBackfillState({
      cursor_json: JSON.stringify({ nextWindowEnd: "2025-05-02T00:00:00.000Z" }),
      indexed_count: 0,
    });
