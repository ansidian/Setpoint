...
import type { EmailRangeResult, NormalizedFetchedEmail } from "../../shared/types/email.ts";
import type { ConfiguredEmailAccount } from "./email-provider-types.ts";
import { emailErrorMessage } from "./email-provider-types.ts";

interface BackfillState extends Record<string, unknown> {
  user_id: string;
  account_id: string;
  mailbox_scope: string;
  cursor_json?: string | null;
  oldest_target_date: string;
  oldest_indexed_date?: string | null;
  indexed_count?: number | string | null;
...
type BackfillStatus = "queued" | "retry" | "paused" | "failed" | "completed" | "completed_empty";
interface BackfillProcessResult {
  processed: boolean;
...
function parseCursor(state: BackfillState): BackfillCursor {
  try {
    return JSON.parse(state.cursor_json || "{}") as BackfillCursor;
  } catch {
    return {};
  }
}

function asTargetDate(value: unknown): Date {
  return new Date(`${String(value).slice(0, 10)}T00:00:00.000Z`);
}

function calculateWindow(state: BackfillState, { now, windowDays }: { now: Date; windowDays: number }): BackfillWindow {
...
function classifyFailure(err: unknown, attempts = 0): "paused" | "failed" | "retry" {
...
function oldestEmailDate(emails: NormalizedFetchedEmail[], fallback: string | null | undefined): string | null {
  const dates = emails
...
async function getNextBackfillState(): Promise<BackfillState | null> {
  ...
  return result.rows[0] as unknown as BackfillState || null;
}

async function markRunning(state: BackfillState, attempts: number): Promise<void> {
  await db.execute({
...
async function markBackfillFailed(state: BackfillState, attempts: number, message: string): Promise<BackfillProcessResult> {
  await db.execute({
    sql: `UPDATE ea_email_backfill_state
          SET status = 'failed',
              last_error = ?,
...
async function loadAccount(state: BackfillState): Promise<ConfiguredEmailAccount | null> {
  const result = await db.execute({
    sql: `SELECT *
          FROM ea_accounts
...
async function fetchProviderWindow(account: ConfiguredEmailAccount, _state: BackfillState, window: BackfillWindow): Promise<EmailRangeResult> {
...
async function persistWindowSuccess(state: BackfillState, window: BackfillWindow, result: EmailRangeResult, { now }: { now: Date }): Promise<BackfillProcessResult> {
  const emails = result.emails || [];
  const nextCursor: BackfillCursor = {
    currentWindow: { start: window.start!, end: window.end! },
  };
  let nextStatus: BackfillStatus = "queued";
...
async function persistWindowFailure(state: BackfillState, window: BackfillWindow, err: unknown, attempts: number): Promise<BackfillProcessResult> {
  const status = classifyFailure(err, attempts);
  if (isInvalidGrantError(emailErrorMessage(err))) {
    try {
      await markAccountNeedsReauth(state.account_id);
    } catch (markErr) {
      console.error("[EA Backfill] Failed to mark needs_reauth:", emailErrorMessage(markErr));
    }
  }
  const cursor = {
    ...parseCursor(state),
  ...
  return { processed: true, status, error: emailErrorMessage(err) };
}

export async function processNextBackfillWindow({
  now = new Date(),
  windowDays = DEFAULT_WINDOW_DAYS,
}: { now?: Date; windowDays?: number } = {}): Promise<BackfillProcessResult> {
...
export async function resumeInterruptedBackfills(): Promise<void> {
  await db.execute({
    sql: `UPDATE ea_email_backfill_state
          SET status = 'retry',
...
export async function enqueueBackfillForAllUsers({ targetDays = DEFAULT_TARGET_DAYS }: { targetDays?: number } = {}): Promise<void> {
  const result = await db.execute({
    sql: "SELECT DISTINCT user_id FROM ea_accounts WHERE type IN ('gmail', 'icloud')",
    args: [],
  });
  for (const row of result.rows) {
    await queueEmailIndexBackfill(String(row.user_id), { targetDays });
  }
}
...
}: { queueOnStartup?: boolean; targetDays?: number } = {}) {
...
async function drainBackfillQueue({ pauseMs = DEFAULT_PAUSE_MS }: { pauseMs?: number } = {}): Promise<void> {
  ...
  try {
    ...
    console.error("[EA Backfill] Worker failed:", emailErrorMessage(err));
  } finally {
    workerRunning = false;
  }
}

export function wakeEmailBackfillWorker({ delayMs = 0, pauseMs = DEFAULT_PAUSE_MS }: { delayMs?: number; pauseMs?: number } = {}): void {
  if (stopping || workerTimer || workerRunning) return;
  workerTimer = setTimeout(() => {
    workerTimer = null;
    drainBackfillQueue({ pauseMs }).catch((err) =>
      console.error("[EA Backfill] Queue drain failed:", emailErrorMessage(err)),
    );
  }, delayMs);
  workerTimer.unref?.();
}
...
export function stopEmailBackfillWorker(): void {
  stopping = true;
  if (workerTimer) {
...
export function startEmailBackfillWorker({
  ...
  targetDays = DEFAULT_TARGET_DAYS,
  queueOnStartup = parseBooleanEnv(process.env.EA_EMAIL_BACKFILL_QUEUE_ON_STARTUP),
}: { initialDelayMs?: number; pauseMs?: number; targetDays?: number; queueOnStartup?: boolean } = {}): void {
  ...
  prepareEmailBackfillStartup({ queueOnStartup, targetDays })
    ...
    .catch((err) => console.error("[EA Backfill] Startup failed:", emailErrorMessage(err)));
}
