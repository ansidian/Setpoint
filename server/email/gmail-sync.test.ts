...
function pubsubBody(payload: Record<string, unknown>, overrides: Record<string, unknown> = {}) {
  return {
    message: {
...
async function seedActiveSnapshotItem(triageId: number, {
  uid = "gmail-gmail-work-msg-1",
  accountId = "gmail-work",
} = {}) {
...
describe("Gmail Pub/Sub sync ingestion", () => {
  it("decodes a push notification and queues one account-level history sync job", async () => {
    ...
    expect(JSON.parse(String(jobs.rows[0]!.payload_json))).toEqual({
      emailAddress: "work@example.com",
      historyId: "9876543210",
...
// CORR-L08 investigation (Step 2): claimNextHistorySyncJob (gmail-sync.ts) already
...
// queue's claimNextEmailTriageJob in triage-job-store.ts. HOWEVER the `job` object
