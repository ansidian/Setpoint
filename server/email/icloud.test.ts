...
const MIME_ALTERNATIVE = {
  text: "You've cancelled autopay for your SoFi Credit Card.",
  ...
  constructor() {
    ...
    this.fetch = vi.fn(async function* () {
      ...
      const alternative = [
        ...
        "--EVIDENCE-ALTERNATIVE", "Content-Type: text/plain; charset=utf-8", "", MIME_ALTERNATIVE.text,
        "--EVIDENCE-ALTERNATIVE", "Content-Type: text/html; charset=utf-8", "", MIME_ALTERNATIVE.html,
        "--EVIDENCE-ALTERNATIVE--",
      ].join("\r\n");
      for (const [index, source] of [htmlSource, cappedSource, alternative].entries()) {
        yield {
          uid: 31 + index,
...
describe("iCloud ingestion evidence", () => {
  ...
  it("indexes the reader HTML and excludes a contradictory plain-text alternative from the preview", async () => {
    const result = await fetchEmailsInRange(account, "password", { start: "2026-05-01", end: "2026-05-02" });
    const email = result.emails.find((item) => item.uid === "icloud-33")!;
    expect(email.body_text).toBe(MIME_ALTERNATIVE.expected);
    expect(email.body_preview).toContain(MIME_ALTERNATIVE.expected);
  });
});
