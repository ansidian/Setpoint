...
// All ea_gmail_watch_state persistence lifted from gmail-sync.ts: watch-state
...
function watchExpirationIso(expiration: string | number): string {
...
export async function persistGmailWatchState(account: GmailSyncAccount, {
  historyId,
  expiration,
  status = "active",
  lastError = "",
  now = new Date(),
}: PersistWatchInput, { dbClient = db }: { dbClient?: EmailWriteDb } = {}): Promise<string> {
...
export async function getStoredHistoryId(account: GmailSyncAccount, dbClient: EmailReadDb): Promise<string | null> {
  ...
  return result.rows[0]?.last_history_id ? String(result.rows[0].last_history_id) : null;
}

export async function markWatchError(account: GmailSyncAccount, message: string, dbClient: EmailWriteDb): Promise<void> {
  await dbClient.execute({
    sql: `INSERT INTO ea_gmail_watch_state
...
export function seedInactiveWatchStateStatement({ account, targetHistoryId, now }: { account: GmailSyncAccount; targetHistoryId: string; now: Date }): InStatement {
  return {
    sql: `INSERT INTO ea_gmail_watch_state
            (user_id, account_id, email_address, last_history_id,
...
export function advanceCursorStatement({ historyId, account, now }: { historyId: string; account: GmailSyncAccount; now: Date }): InStatement {
  return {
...
export function touchCursorStatement({ account, now }: { account: GmailSyncAccount; now: Date }): InStatement {
  return {
    sql: `UPDATE ea_gmail_watch_state
          SET last_sync_at = ?,
              last_error = ?,
