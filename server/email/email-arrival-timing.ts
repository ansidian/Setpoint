...
type TimestampState = "missing" | "invalid" | "valid";
type TimestampField = "providerPublishedAt" | "historyQueuedAt" | "historyClaimedAt" | "snapshotQueuedAt" | "completedAt";
interface ParsedTimestamp { ms: number | null; state: TimestampState }
interface EmailArrivalTimingInput {
  providerPublishedAt?: unknown;
  historyQueuedAt?: unknown;
  historyClaimedAt?: unknown;
  snapshotQueuedAt?: unknown;
  completedAt?: unknown;
}
...
export function projectEmailArrivalTiming({
  ...
  const duration = (startField: TimestampField, endField: TimestampField): number | undefined => {
