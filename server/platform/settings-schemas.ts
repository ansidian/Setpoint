...
function isValidTimeZone(tz: unknown) {
...
function parseMaybeJson(value: unknown, label: string): ParseResult {
...
export function validateSchedules(value: unknown) {
  ...
  // identity scheduler.ts keys skip-checks/re-registration on stays unique —
  ...
  // note after merge.
  const seenTimeLabel = new Set<string>();
...
export function validateEmailInterests(value: unknown) {
...
export function validateImportantSenders(senders: unknown) {
  ...
  for (const sender of senders) {
    ...
    if (
      sender.source !== undefined
      && (typeof sender.source !== "string" || !IMPORTANT_SENDER_SOURCES.has(sender.source))
    ) {
...
export function validateActualBudgetUrl(value: unknown) {
...
export function validateDiscordWebhookUrl(value: unknown) {
  const trimmed = typeof value === "string" ? value.trim() : "";
  if (trimmed === "") return { valid: true, value: "" };
  let parsed;
  try {
...
export function validateUtilityPayLinks(links: unknown) {
  if (!Array.isArray(links)) {
    return { valid: false, message: "utility_pay_links must be an array" };
  }
  const seen = new Set<string>();
  const value: Array<{ scheduleId: string; label: string; url: string }> = [];
