...
type DatabaseOptions = { dbClient?: Client };
 ...
 */
export function isInvalidGrantError(errOrText: unknown) {
 ...
 * @returns {Promise<Object>} result with rowsAffected
 */
export async function clearAccountNeedsReauth(
  accountId: string,
  { dbClient = db }: DatabaseOptions = {},
) {
  const result = await dbClient.execute({
 ...
 * @returns {Promise<Object>} result with rowsAffected
 */
export async function markTodoistNeedsReauth(
  userId: string,
  { dbClient = db }: DatabaseOptions = {},
) {
  const result = await dbClient.execute({
 ...
 * @returns {Promise<Object>} result with rowsAffected
 */
export async function clearTodoistNeedsReauth(
  userId: string,
  { dbClient = db }: DatabaseOptions = {},
) {
  const result = await dbClient.execute({
