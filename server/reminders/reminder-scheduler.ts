...
type ReminderSendFn = (webhookUrl: string, payload: DiscordWebhookPayload, context: { reminder: Reminder }) => Promise<ReminderDeliveryLike>;
interface ProcessDueReminderBatchOptions {
  now?: DateInput;
  limit?: number;
  dbClient?: Client;
...
async function loadDiscordSettings(userId: string, { dbClient = db }: { dbClient?: Client } = {}): Promise<DiscordSettings | null> {
  ...
  return row ? {
    discord_webhook_url_encrypted: row.discord_webhook_url_encrypted == null ? null : String(row.discord_webhook_url_encrypted),
    discord_user_id: row.discord_user_id == null ? null : String(row.discord_user_id),
  } : null;
}
...
function retryAfterFromResult(result: { rateLimited?: boolean; retryAfterMs?: number | null }, now: DateInput, retryCount = 0): string {
...
function computeReminderRetryAfter(reminder: Reminder, now: DateInput): string {
  return retryAfterFromResult({ rateLimited: false }, now, Number(reminder.retry_count) || 0);
}

function publishReminderChange(reminder: Reminder, reason: string, state = "current"): void {
...
export async function processDueReminderBatch({
  ...
  const settingsByUser = new Map<string, DiscordSettings | null>();
  const webhookByUser = new Map<string, string>();
  const getDiscordSettings = async (userId: string): Promise<DiscordSettings | null> => {
