...
export function triageRetryBackoffIso(now, attempts) {
...
export async function claimNextEmailTriageJob(dbClient, now) {
  const result = await dbClient.execute({
    sql: `SELECT *
          FROM ea_triage_jobs
...
export async function requeueClaimedJob(job, dbClient) {
...
export async function completeJob(job, dbClient, now, lastError = "") {
  await dbClient.execute({
    sql: `UPDATE ea_triage_jobs
          SET status = 'complete',
              completed_at = ?,
...
export async function deferJob(job, dbClient, scheduledFor, lastError = "") {
  await dbClient.execute({
    sql: `UPDATE ea_triage_jobs
          SET status = 'queued',
              locked_at = NULL,
