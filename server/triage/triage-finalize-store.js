...
import { nowIso } from "./triage-job-store.js";

// ea_email_triage + ea_briefing_snapshot_items persistence lifted from
// triage-worker.js: load the email for a job, write the triage decision row,
...
export async function loadEmailForJob(job, dbClient) {
...
export async function updateTriageRow(email, decision, {
  dbClient,
  now,
  status = "complete",
...
export async function attachToActiveSnapshot(email, decision, { dbClient, now }) {
...
export async function delayWeakSecurityGrace(job, email, preflight, { dbClient, now }) {
