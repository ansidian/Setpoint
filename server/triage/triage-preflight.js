...
export function loadDefaultPreflightRules(path = DEFAULT_RULES_PATH) {
  let rules;
  try {
    rules = JSON.parse(readFileSync(path, "utf8"));
  } catch (err) {
    throw new Error(`Failed to load triage preflight rules from ${path}: ${err.message}`);
  }
  ...
  for (const rule of rules) {
    ...
    if (!rule.match_json || typeof rule.match_json !== "object") {
      throw new Error(`Triage preflight rule "${rule.key}" has no match_json object (${path})`);
    }
    for (const field of ["subject_regex", "body_regex"]) {
      if (rule.match_json[field] === undefined) continue;
      try {
        new RegExp(rule.match_json[field], "i");
      } catch (err) {
        throw new Error(`Triage preflight rule "${rule.key}" has an invalid ${field}: ${err.message}`);
      }
    }
  }
  return rules;
