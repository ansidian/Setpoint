...
export async function routeEmailForTriage(email: TriageEmail, {
  ...
  return withAiUsageContext({
    userId: email.user_id, origin: "background_triage", accountId: email.account_id, emailId: email.email_id,
  }, async () => {
...
export async function processNextEmailTriageJob({
  ...
  return withAiUsageContext({
    userId: job.user_id, origin: "background_triage", accountId: job.account_id, emailId: job.email_id,
  }, async () => {
