...
import type { TriageDecision, TriageLane } from "./triage-types.ts";
...
function scoreEmail(email: Record<string, unknown>): { score: number; signals: string[] } {
...
function laneForScore(score: number): TriageLane {
  if (score >= HEURISTIC_HIGH) return "needs_attention";
  if (score <= HEURISTIC_LOW) return "noise";
  return "fyi";
}

export function heuristicNoModelDecision(email: Record<string, unknown>, _opts: Record<string, never> = {}): TriageDecision {
  try {
    ...
    return createTriageDecision({
      ...
      summary: String(email?.body_snippet || email?.subject || "Review provider message."),
