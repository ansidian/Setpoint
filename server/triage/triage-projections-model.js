...
// Pure, DB-free triage projections lifted from triage-worker.js so they can be
...
export function emailSearchText(email) {
  return [
    email.from_name,
    email.from_address,
    email.subject,
...
export function normalizeEmailInterests(raw) {
...
export function triageSoundTriggerType(reason, lane) {
...
export function emailTriageEventDetails(email, { reason, lane, triageSource }) {
  return {
...
export function maybeBillCandidate(email, decision) {
...
export function weakSecurityReadDecision() {
