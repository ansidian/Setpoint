...
type EvalFamily = (typeof FAMILIES)[number];

interface FamilySuggestions {
  family: string;
  queries: string[];
  samples: Array<Record<string, unknown>>;
}

function whereForTerms(terms: string[]): string {
  return terms.map(() => "(lower(idx.subject) LIKE ? OR lower(idx.body_snippet) LIKE ?)").join(" OR ");
}

async function sampleFamily(family: EvalFamily): Promise<FamilySuggestions> {
  if (!userId) throw new Error("EA_USER_ID or explicit userId argument is required");
...
main().catch((err: unknown) => {
  console.error("[Email Search Retrieval Eval] Seed failed:", err instanceof Error ? err.message : err);
  process.exit(1);
});
