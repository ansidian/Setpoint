...
// Usage: node server/scripts/reindex-icloud-mime.ts [--apply]
...
interface PollutedEmailRow {
  uid: string;
  email_date_utc: string;
}

async function scanPolluted(): Promise<PollutedEmailRow[]> {
  const polluted: PollutedEmailRow[] = [];
  const pageSize = 250;
  for (let offset = 0; ; offset += pageSize) {
    const { rows } = await db.execute({
      ...
      args: [requiredUserId, pageSize, offset],
    });
    if (!rows.length) break;
    for (const row of rows) {
      if (looksLikeRawMime(String(row.body_text ?? ""))) {
        polluted.push({
          uid: String(row.uid ?? ""),
          email_date_utc: String(row.email_date_utc ?? ""),
        });
      }
    }
  }
...
const { accounts } = await loadUserConfig(requiredUserId);
...
await indexEmails(requiredUserId, emails);
