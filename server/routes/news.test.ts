...
describe("news routes", () => {
  ...
  it("POST /topics creates a topic at the end position", async () => {
    ...
    expect(page.body.topics.map((t: { name: string }) => t.name)).toEqual(["AI", "Tech"]);
  });
  ...
  it("POST /topics/import-starter copies chosen catalog bundles", async () => {
    ...
    expect(res.status).toBe(200);
    const page = await request(app).get("/api/news");
    expect(page.body.topics.map((t: { name: string }) => t.name)).toEqual(["AI", "PC Hardware"]);
    const ai = page.body.topics.find((t: { name: string }) => t.name === "AI");
    expect(ai.sources.some((s: { kind: string; hnQuery: string | null }) => s.kind === "hn" && s.hnQuery === "AI")).toBe(true);
  });
  ...
  it("POST /topics/reorder rewrites positions", async () => {
    ...
    const ids = page1.body.topics.map((t: { id: number }) => t.id).reverse();
    await request(app).post("/api/news/topics/reorder").send({ ids });
    const page2 = await request(app).get("/api/news");
    expect(page2.body.topics.map((t: { name: string }) => t.name)).toEqual(["B", "A"]);
  });
  ...
  it("DELETE /topics/:id removes the topic, its sources, and their items", async () => {
    ...
    for (const table of ["ea_news_topics", "ea_news_sources", "ea_news_items"]) {
      const count = (await currentDb().execute(`SELECT COUNT(*) AS n FROM ${table}`)).rows[0]!.n;
  ...
  it("POST /sources creates rss and hn sources; PATCH toggles; DELETE removes with items", async () => {
    ...
    const after = await request(app).get("/api/news");
    expect(after.body.topics[0].sources.find((s: { id: number }) => s.id === rss.body.source.id).enabled).toBe(false);
  ...
  it("GET /catalog returns the starter catalog", async () => {
    ...
    expect(res.status).toBe(200);
    expect(res.body.topics.map((t: { name: string }) => t.name)).toContain("3D Printing");
  });
});
