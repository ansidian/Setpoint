...
let tempDir = null;
let originalDataDir = process.env.ACTUAL_DATA_DIR;
...
describe("sendBillLightweight", () => {
  ...
  it("writes a future bill schedule into the local Actual DB and syncs schedule CRDT messages", async () => {
    ...
    expect(JSON.parse(rules.rows[0].conditions)).toEqual([
    ...
    expect(JSON.parse(rules.rows[0].actions)).toEqual([
  ...
  it("updates an existing future bill schedule instead of creating duplicates", async () => {
    ...
    expect(JSON.parse(rules.rows[0].conditions)).toEqual(expect.arrayContaining([
  ...
  it("does not flag pre-write failures as locally applied", async () => {
    ...
    expect(err.localWriteApplied).toBeUndefined();
  });
});
