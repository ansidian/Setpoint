...
function nameMap(rows: Row[]): Record<string, string> {
  return Object.fromEntries((rows || []).map((r) => [String(r.id), String(r.name || "")]));
}

function resolveName(value: string | undefined, rows: Row[]): { ok: true; id: string | null } | { ok: false } {
  if (value == null || value === "") return { ok: true, id: null };
  const target = String(value).toLowerCase();
  const match = (rows || []).find((r) => String(r.name || "").toLowerCase() === target);
  return match ? { ok: true, id: String(match.id) } : { ok: false };
}
...
// is missing.
export async function readTransactionsRange(
  userId: string,
  ...
  const limit = typeof filters.limit === "number" && Number.isFinite(filters.limit) ? filters.limit : DEFAULT_LIMIT;
  ...
  try {
    ...
    const checks: Array<[string, ReturnType<typeof resolveName>, string | undefined]> = [
    ...
    const [payeeF, categoryF, accountF] = checks.map(([, res]) => res.ok ? res.id : null);
    ...
    const args: Array<string | number> = [actualDateInt(start), actualDateInt(end)];
    ...
    if (typeof minAmount === "number" && Number.isFinite(minAmount)) { clauses.push("ABS(t.amount) >= ?"); args.push(Math.round(minAmount * 100)); }
    if (typeof maxAmount === "number" && Number.isFinite(maxAmount)) { clauses.push("ABS(t.amount) <= ?"); args.push(Math.round(maxAmount * 100)); }
    if (notes) {
    ...
    const all = result.rows.map((r) => {
      ...
      return {
        ...
        transferAccountId: payeeTransferAccount[String(r.payee)]
          ? String(payeeTransferAccount[String(r.payee)])
          : null,
