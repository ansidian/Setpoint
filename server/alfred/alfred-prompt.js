...
export function buildAlfredSystemPrompt({ now = new Date() } = {}) {
  ...
  return [
    ...
    "Before writing a reply that names specific emails, events, deadlines, or bills — even a single one — first call show_items with their ids so the owner sees real data rows. Then keep prose to a line or two around the rows; never retype amounts, dates, times, locations, or senders that the rows already show.",
