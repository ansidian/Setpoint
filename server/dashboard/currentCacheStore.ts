...
// All ea_current_data_cache reads/writes lifted from current-service.ts: load the
...
export async function loadCacheRows(
  userId: string,
  ...
  return Object.fromEntries(result.rows.map((row) => [String(row.cache_key), row])) as CurrentDashboardCacheRows;
}

export async function saveCacheRow(userId: string, cacheKey: CurrentDashboardCacheKey, payload: unknown, {
  dbClient = db,
  now = new Date(),
  status = "current",
  errorMessage = null,
...
export async function markCacheRowRefreshFailed(userId: string, cacheKey: CurrentDashboardCacheKey, err: unknown, {
  dbClient = db,
  now = new Date(),
  existingRow = null,
}: {
  dbClient?: Client;
  now?: Date;
  existingRow?: CurrentDashboardCacheRow | null;
} = {}): Promise<CurrentDashboardCacheRow> {
  const timestamp = now.toISOString();
...
export async function markRowsRefreshing(
  userId: string,
