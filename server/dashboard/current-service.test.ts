...
describe("GET /api/dashboard/current", () => {
  ...
  it("returns stale cached rows immediately while refreshing them in the background", async () => {
    ...
    let resolveWeather: ((value: Record<string, unknown>) => void) | undefined;
    let markWeatherStarted: (() => void) | undefined;
    const weatherStarted = new Promise<void>((resolve) => {
      markWeatherStarted = resolve;
    });
    const weatherRefresh = new Promise<Record<string, unknown>>((resolve) => {
