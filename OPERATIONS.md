...
2. Review the inventory, then repeat with `--apply --fingerprint <exact fingerprint>`. Concurrent legacy changes reject the migration. IDs, target permissions and utility history survive; the verified Citi statement sender alias and matching merchant label are explicitly corrected. Thereafter the unified revision/budget-bound API owns edits; stale legacy saves return a conflict.
