...
8. `server/triage/triage-worker.ts:processNextEmailTriageJob` reads the persisted provider-neutral authentication projection, plans every new triage financial candidate, then `triage-finalize-store.ts:updateTriageRow` atomically persists the candidate and complete plan. Claimed sender text is never promoted to authentication evidence.
