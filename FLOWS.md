...
6. `server/transaction-imports/transaction-import-worker.ts` — resumes Gmail page cursors for historical scans, prepares planner-owned candidate batches, contains unresolved targets or unsupported currencies in review, and performs an Actual dry run before any commit. An enabled planner-owned expense or income transaction becomes ready/automatic-safe only after a would-add preview, a previously passed current-Actual duplicate check, and all required plan gates; generic outcomes also update the triage plan. Provider-reference identities converge pending/completed lifecycle notices, with message identity as fallback. Expenses remain negative, income remains positive, and the Actual import Adapter writes both uncleared. Duplicates stop, updates stay in review, and unknown prior reconciliation cannot be replaced by an imported-ID-only preview. Existing observe items still require confirmation. Historical source-specific items continue from their captured targets/modes without consulting the retired mapping table.
...
11. `server/routes/briefing/transaction-imports.ts` — exposes owner-scoped run, confirm, retry, dismiss, and resume controls behind briefing cookie authentication; clients poll durable run/item state and redacted planner evidence. Source configuration routes and client contracts are removed.
...
**UI:** Inbox reader status and review actions (`src/components/inbox/reader/TransactionImportStatus.tsx`) plus the financial scan/run/review workflow (`src/components/financial/FinancialBackfill.tsx` and `src/components/financial/PendingFinancialRecord.tsx`) consume the authenticated transaction-import routes. There are no source automation-mode or account/category mapping controls.
...
Dashboard review previews open the shared foreground with exact import item and `recordRun` identity; the run filter remains independently encoded by `runId`. Compatible `reviewPending=1` Settings links remain available. Its `GET /api/dashboard/finance/review-runs?offset=…` pages twelve pending runs using the same eligibility predicate as the dashboard count. Existing import detail/confirmation/retry/dismiss endpoints remain the write owners. Source email evidence is available inside the financial record through the existing body-fetch and safe iframe seam. Demo endpoints project the same in-memory transaction seed as Calendar, and confirmed demo imports update that seed.
...
Build-time demo mutations update one exact-ID, in-memory financial projection. Managed completion, historical confirmation and correction update ledger/bills, spending, current Dashboard, Inbox status and shared history. One local `ea-demo-financial-changed` event enters the same dashboard change handler without creating an EventSource. Transfer pairs remain inspectable in raw correction evidence but are excluded from the normal Calendar transaction range and spending, matching production. Duplicate confirmation is inert, refresh resets state, and no provider/network/persistence fallback exists.
