...
9. `server/bills/financial-email-adoption-service.ts:resolveFinancialEmailSeed` returns a stored valid plan without provider work unless newly indexed authentication is stronger than the stored plan. Historical candidates then refresh through the same planner and compare-and-swap persistence; exact winners are idempotently staged for Actual preflight. Existing stored observe-only plans retain that mode; rollout does not bulk-retriage or promote them.
