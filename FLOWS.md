...
3. `server/email/gmail.ts` → `server/email/sender-authentication.ts:evaluateGmailSenderAuthentication` normalizes the first trusted Gmail receiver verdict into a redacted provider-neutral projection; copied or untrusted results remain unavailable. iCloud explicitly remains unavailable. `server/email/email-index.ts:indexEmails` persists that projection alongside the email, then `server/email/verification-code-detector.ts:detectVerificationCode` atomically replaces nullable code metadata without an external/model call or persisted evidence
