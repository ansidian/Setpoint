...
2. A missing candidate uses the existing first-pass extractor. Triage and extraction share source-grounded payment purpose (`type`, confidence and evidence) and a verbatim card-product hint. A persisted complete candidate skips provider work; missing/uncertain event semantics or a legacy scheduled-payment candidate missing purpose receives a bounded verification pass. Completed type audits are reused; failed audits may retry on read after five minutes, up to three total attempts, then require re-triage.
   The shared extraction instructions use `due_date` as the operation date: explicitly evidenced transaction/payment dates for completed events, due dates for obligations, and scheduled payment dates for upcoming payments. Missing or ambiguous dates remain null; completed transaction dates do not become Inbox deadlines. Reopening a legacy candidate does not rerun first-pass extraction.
...
7. `server/bills/statementActualStatusModel.ts:resolveStatementActualStatus` reconciles the inferred plan, including completed purchases. Exact duplicates become no-write; a same-schedule amount/date-only mismatch is represented as `disposition: "update_existing"` without expanding the operation union; other conflicts remain review. `server/bills/financialEmailAutomationPolicy.ts:financialEmailAutomationEligibility` projects semantic, amount/date, target, normalized sender-authentication, stable-identity, warning/currency, reconciliation, Actual-preflight, and per-class rollout gates. One-time expenses and transfer schedules are enabled; income and utility schedules remain observe-only. Missing/failed authentication or non-USD/unknown currency blocks transaction automation.
...
9. `server/bills/financial-email-adoption-service.ts:resolveFinancialEmailSeed` returns a stored valid plan without provider work unless newly indexed authentication is stronger, a legacy payment purpose is eligible for bounded verification, or an unresolved transaction payee predates the current target-inference version. Historical candidates then refresh once through the same planner and compare-and-swap persistence; exact winners are idempotently staged for Actual preflight, while the persisted inference version prevents repeated matching calls after an unresolved refresh. New USD transfer schedules use a shared payment identity across reminder messages and a separate create-only executor. Existing stored observe-only plans retain that mode; rollout does not bulk-retriage or promote them.
...
6. `server/transaction-imports/transaction-import-worker.ts` — resumes Gmail page cursors for historical scans, prepares planner-owned candidate batches, contains unresolved targets or unsupported currencies in review, and performs an Actual dry run before any commit. An enabled planner-owned expense becomes ready/automatic-safe only after a would-add preview, a previously passed current-Actual duplicate check, and all required plan gates; generic outcomes also update the triage plan. Duplicates stop, updates stay in review, and unknown prior reconciliation cannot be replaced by an imported-ID-only preview. Existing observe items still require confirmation. Historical source-specific items continue from their captured targets/modes without consulting the retired mapping table.
