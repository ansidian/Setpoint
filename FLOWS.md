...
9. `server/bills/financial-email-adoption-service.ts:resolveFinancialEmailSeed` returns a stored valid plan without provider work unless newly indexed authentication is stronger or a legacy payment purpose is eligible for bounded verification. Historical candidates then refresh through the same planner and compare-and-swap persistence; exact winners are idempotently staged for Actual preflight. New USD transfer schedules use a shared payment identity across reminder messages and a separate create-only executor. Existing stored observe-only plans retain that mode; rollout does not bulk-retriage or promote them.
