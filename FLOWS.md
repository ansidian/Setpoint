...
## 1. Actual record → Actual sync → metadata invalidation

**Reader entrance:** every email exposes one **Actual record** action, independent of triage lane, `hasBill`, snooze or historical snapshot state. `ActualRecordWorkspace.tsx` waits for `useBillPayResolver.ts` to resolve ownership. Managed records open the revision-checked completion form; inline reader status stays informational. Historical records also require a successful email-import-status lookup: existing import items retain their original status/review path, and an attempted write, existing record or conflicting Actual match cannot expose another writer. Lookup failures offer Retry. Recorded items keep **View in Calendar** inside this workspace when the target is available.

**Historical manual trigger:** user confirms a record in the retained form (`src/components/bills/useBillBadgeForm.ts:handleSend`) → POST `/api/briefing/actual/send` handled in `server/routes/briefing/bills.ts`. The semantic `no_write` assessment alone does not veto explicit owner completion. `manualActualRecordPolicy.ts` preserves known write/reconciliation ownership; this historical endpoint retains its existing write/retry semantics.
...
Migration 062 atomically admits only newly indexed, post-cutoff email into `ea_financial_documents`; existing rows and historical backfill are excluded. New Gmail received-mail acquisition also indexes mail outside Inbox using a durable forward cursor; iCloud remains Inbox-scoped. Pending capture must finish before related events can write. `server/transaction-imports/transaction-import-runtime.ts` drains capture, source documents, financial events and retained legacy work in bounded batches.

1. `server/triage/financial-document-classifier.ts` assesses the complete bounded source with the configured strong model and semantic verifier, independently of Inbox rules and state. Explicit nonfinancial results are recorded. Provider failures retry; failed verification, contradictory purpose and unsupported operation dates receive up to three fresh full-source assessments before caching. Unchanged successful source assessments are reused.
2. `server/financial-events/financial-event-evidence.ts` groups exact grounded provider references through permanent aliases. A unique authenticated merchant receipt and processor/bank confirmation can combine matching merchant/amount/currency/date and complementary funding evidence within five minutes. Conflicting facts or competing purchases remain unresolved; bounded history cannot erase a prior ambiguity.
3. The event worker collects arrivals for 90 seconds, then calls the shared planner with complete authenticated source bodies and current Actual targets. Oversized combined evidence fails visibly at the existing 20,000-character decision limit. Explicit source account evidence outranks merchant habits; grounded new payees and uncategorized entries are allowed. A missing transaction date is never invented. Completed payment amounts cannot be replaced by statement balances or minimum due.
4. `financial-event-operation.ts` previews transactions, completed paired transfers, utility schedules and existing transfer schedules through the SDK facade. The current budget and any schedule fingerprint are bound into an immutable payload and attempt before dispatch. All later claims only recover that same operation, including crashes, transport failures and new source evidence.
5. Successful entries settle once per event. Missing evidence/temporary failures retry; definitive Actual conflicts require attention. Late contradictory evidence preserves the prior entry and reports the conflict instead of presenting the old write as confirmation of changed facts.
6. `financial-event-status.ts` supplies reader status without model work or a second write path. The reader shows collecting/checking, waiting, recorded/scheduled, or attention required with related-email count. Managed emails skip the old triage planner, generic preflight, source-parser admission paths. Existing saved import records remain readable.
7. The reader's **Actual record** workspace opens the completion form and submits owner-confirmed facts to `POST /api/briefing/financial-events/complete`. Source/event revisions are captured when editing begins and checked atomically when accepted. The same event retains the original source evidence and the owner's entry, invalidates competing leases, and queues deterministic planning even while AI is paused. Current Actual validation, capture completion and immutable attempt admission still apply. Waiting or review records can be corrected before an attempt; attempted or confirmed writes cannot be resubmitted. Completion responses refresh the reader caches and resume status polling without claiming that a queued entry is already recorded.

### Shared financial-email planning and retained historical flows

Email evidence is normalized before indexing by `server/email/email-evidence.ts`: Gmail selects one readable MIME alternative (plain text preferred), HTML retains blocks and table columns, and tracking URL noise is removed before the 20,000-character storage bound. Oversized stored bodies carry an explicit incompleteness marker. Triage checks completeness before heuristics or preflight finalization and uses the whole bounded body; incomplete evidence lands in Needs Review without a financial candidate. `bill-extract.ts` uses the same full-document boundary instead of keyword selection or a 3,500-character prefix. Multi-amount verification audits ungrounded/conflicting label evidence even with complete numeric coverage; failed audits cannot supply an operational canonical amount. Existing flattened rows require an explicit provider refetch (`npm run email:reindex-evidence -- --uid <uid>`, dry-run by default); index refresh leaves saved triage and plans unchanged.
...
2. A missing candidate uses the existing first-pass extractor. Triage and extraction share source-grounded payment purpose (`type`, confidence and evidence), a verbatim card-product hint, and distinct verbatim source/destination hints for non-card account transfers. Triage candidates use a complete nullable semantic shape. A persisted complete candidate skips provider work; missing/uncertain event semantics, a visible-but-unparsed currency value or operation date, or a legacy scheduled-payment candidate missing purpose receives a bounded verification pass. Completed type audits are reused; failed audits may retry on read after five minutes, up to three total attempts, then require re-triage.
   The shared extraction instructions use `due_date` as the operation date: explicitly evidenced transaction/payment/order/redemption dates for completed events, due dates for obligations, and scheduled payment dates for upcoming payments. Missing or ambiguous dates remain null; completed transaction dates do not become Inbox deadlines. The model also emits grounded settlement kind and provider reference. Non-card wallet/bank transfers have explicit pending/completed event kinds; the owner-approved PayPal-balance-to-bank case is treated as external Cashback income, while confirmed movements between represented Actual accounts propose paired transfers for the new event executor.
...
5. `server/bills/financialEmailTargetInference.ts:inferFinancialEmailTargets` resolves targets from unique compatible last-four, exact card-product or schedule/payee identity, two-or-more fully consistent direction-aware history rows, conditional category history, then constrained ranking. Grounded Cashback income may select unique Actual `Cashback` payee/category targets. Statement credits use the evidenced source-card suffix; bank deposits use an explicit destination suffix or a unique stable bank destination from direction-compatible Cashback history. Duplicates remain unresolved. When a bank merchant descriptor is not an exact payee name, `server/bills/financialEmailMerchantCandidates.ts:discoverCorroboratedMerchantHistory` retrieves a bounded shortlist from real Actual payees plus repeated compatible history; only the constrained ranker can select a non-exact candidate. Transfer destinations can be selected only from existing Actual accounts; funding comes from corroborating schedules/history. Schedule signs distinguish source-side outflows from destination-side inflows; reversed or unknown directions cannot satisfy a payment. Conflicting exact evidence stays review. Candidate-carried target IDs are cleared and never treated as target evidence.
6. `server/bills/financialEmailTargetRanker.ts:rankFinancialTargetBundles` exposes only opaque keys and owner-facing bundle descriptions to the provider; invalid keys, confidence below 0.8, non-verbatim evidence, or provider failure remain unresolved. History bundles contain account/payee identity only; category disagreement cannot split a bundle or block resolved targets. Categories are optional for every planning source and Actual write path. Deterministic category evidence may prefill a target, but missing or ambiguous evidence leaves it unset. The ranker does not infer categories. Unavailable category IDs are omitted, and schedule updates preserve existing categories when no replacement is supplied.
7. `server/bills/statementActualStatusModel.ts:resolveStatementActualStatus` reconciles the inferred plan, including completed purchases, rewards, refunds, and external inflows. Exact duplicates become no-write; a same-schedule amount/date-only mismatch is represented as `disposition: "update_existing"` without expanding the operation union; other conflicts remain review. `server/bills/financialEmailAutomationPolicy.ts:financialEmailAutomationEligibility` projects semantic, amount/date, target, normalized sender-authentication, stable-identity, warning/currency, reconciliation, Actual-preflight, and per-class rollout gates. The new event executor supports one-time expenses, income, transfer schedules, utility schedules and completed transfers; older import paths retain their supported operation boundaries. Missing/failed authentication or non-USD/unknown currency blocks transaction automation.
8. `server/triage/triage-worker.ts:processNextEmailTriageJob` reads the persisted provider-neutral authentication projection, plans financial candidates outside the managed new-arrival workflow, then `triage-finalize-store.ts:updateTriageRow` atomically persists the candidate and complete plan. Claimed sender text is never promoted to authentication evidence. Exact USD one-time expenses and transfer schedules whose prerequisite gates pass are staged into the existing import lifecycle. Enabled plans enter automatic mode, but are not automatic-safe until preview succeeds.
9. `server/bills/financial-email-adoption-service.ts:resolveFinancialEmailSeed` returns a stored valid plan without provider work unless newly indexed authentication is stronger, a legacy payment purpose is eligible for bounded verification, a blocked plan predates the current candidate-semantics contract, or an unresolved transaction payee or category-only blocker predates the current target-inference version. A stale blocked semantic plan reruns first-pass extraction from the stored email body; other historical candidates refresh through the same planner and compare-and-swap persistence. Provider failure preserves the prior plan, and persisted version markers prevent repeated successful upgrades. Exact winners are idempotently staged for Actual preflight. New USD transfer schedules use a shared payment identity across reminder messages and a separate create-only executor. Existing stored observe-only plans retain that mode; rollout does not bulk-retriage or promote them.
...
11. `src/components/inbox/reader/useBillPayResolver.ts` caches historical plans and refreshes pending/waiting managed status; `BillBadge.tsx` consumes its purpose, inferred destination, reconciliation, and review reasons while touched fields remain owner-controlled.

**Writes:** the new event worker owns managed arrivals. Planning and `server/bills/financial-email-observe-report.ts:readFinancialEmailObserveReport` never write to Actual. The report retains historical plan summaries and adds complete document/event outcome counts, including failures that never produced a plan. `src/components/bills/useBillBadgeForm.ts:handleSend` remains the historical bill-form write trigger, and no-write plans keep that action disabled. Qualified historical transfer plans can also enter the durable automatic executor described in Flow 12.

**Retired configuration:** Bill Pay Settings contracts/cards, `/bills/resolve-sample`, internal mapping resolvers, and transaction-import source configuration are removed. Stored `bill_pay_mappings_json` and `ea_transaction_import_mappings` data remains untouched for audit/recovery, with no live reads or fallback. New imports use planner-owned targets and eligibility; historical import items preserve captured targets and execution modes. Utility pay links remain owner configuration.
...
8. `server/triage/triage-worker.ts:routeEmailForTriage` — preflight rules, then cheap-model classification with strong-model escalation. Built-in receipt, refund, and autopay rules route through semantic classification before financial candidate admission. The model distinguishes actual recipient financial events from incidental money mentions and preserves real events with missing details.
9. `server/triage/triage-finalize-store.ts:updateTriageRow` — persists the decision (lane, summary, bill candidate) to `ea_email_triage`. Only an explicit semantic candidate is persisted; rule/failure fallbacks never synthesize one from keywords or dollar amounts. Existing saved candidates are unchanged until re-triage.
...
1. Triage worker/classification, manual extraction, reader adoption, and planner entry points establish an async-scoped owner/origin/run identity. Nested work inherits it; evaluation entry points mark an evaluation context that bypasses usage preparation and database recording entirely while preserving the provider result/error. Real triage evaluations require `EA_USER_ID` or an explicit owner to load model settings, never a fixture-label owner.
2. `server/platform/ai-usage.ts:trackedAiProviderCall` wraps each actual HTTP attempt in triage and bill provider adapters. Shared initial classification is `triage_cheap` / `triage_strong`; additional calls are `extraction`, `verification`, or `matching`. Deterministic verification and cached candidate/plan reuse create no events. Actual retries and concurrent attempts have independent UUIDs even when only one plan wins persistence.
3. Returned envelope usage is saved before semantic parsing; the same event then becomes succeeded or parse-error. Transport/HTTP/envelope failures retain an attempt with unknown usage. A rejected semantic ranking is a successful billed provider call, not a provider failure. Later workflow failure never deletes usage. Provider latency ends at envelope capture; existing triage latency remains end-to-end classification/verification time.
4. `ai-usage-tokens.ts` normalizes total input including cache reads/writes (Anthropic reports these separately; OpenAI includes them). Standard text price estimates are snapshotted with a version; unknown models/tiers, unsupported long contexts, and missing/invalid usage remain unpriced. No prompts, bodies, candidate fields, credentials, or raw error strings enter the ledger.
5. Migration `057_email_ai_usage.sql` freezes only the old OpenAI latest-row usage projection and establishes the ledger boundary. `getTriageCacheStats` now reads that legacy snapshot, never mutable email rows. It remains incomplete history and is never added to event totals. No past extraction, dry-run, retry, or evaluation usage is invented.
6. Authenticated `GET /api/ea/email-ai/usage` rolls up the owner's last seven days by context, category, purpose and model. Triage and Financial email tabs display only production usage, with no context selector, and label missing usage, unpriced subtotals, and unfinished outcomes. Existing evaluation records remain separate in the ledger/API and are excluded from these views; new tagged evaluation calls are not recorded. Usage details contains the ledger start date and accounting explanations. Analytics does not fetch or display the legacy snapshot. Alfred and Email Search remain on their existing paths. Demo returns fictional in-memory data only.
...
**Triggers:** Gmail history sync indexes a newly arrived message (`server/email/gmail-sync.ts:syncGmailHistoryForAccount`).
...
2. `server/transaction-imports/transaction-import-service.ts:createTransactionImportService` — parses recognized merchant messages and their normalized authentication projections into canonical candidates or review-safe rejections without loading source modes or mappings. Missing, failed, or unavailable authentication is a blocking warning; an exact sender string alone is not authentication. Deterministic receipt ownership in `server/transaction-imports/parsers/parser-registry.ts` excludes these messages from generic preflight, preventing two importer identities for one receipt.
3. `server/transaction-imports/transaction-import-planner-adapter.ts` → `server/bills/financial-email-planner.ts` — adapts exact parser identity/cents/warnings and the redacted authentication verdict into the shared financial plan, infers targets only from Actual evidence, and applies planner rollout to new items. The persisted `executionOwner: "planner"` marker distinguishes these items from historical target/mode snapshots. Provider/body excerpts are redacted; a planner failure retains the canonical item in review.
4. `server/transaction-imports/transaction-import-store.ts` — owns arrival progress, candidate identity uniqueness, captured targets/modes, redacted financial plans, historical comparisons, conditional claims, retry ceilings, stale-claim recovery, manual corrections, and owner-scoped reads in `ea_transaction_import_runs` and `ea_transaction_import_items`. Generic financial-email rows use a partial unique imported-identity index. The retained mapping table has no runtime reader or writer.
5. `server/transaction-imports/transaction-import-runtime.ts` — admits immediate drains and uses a five-minute safety backstop and a 30-second saturated-drain recheck; shutdown stops admission and awaits already-running work without rewriting durable state.
6. `server/transaction-imports/transaction-import-worker.ts` — processes arrival-owned candidate batches, contains unresolved targets or unsupported currencies in review, and performs an Actual dry run before any commit. An enabled planner-owned expense or income transaction becomes ready/automatic-safe only after a would-add preview, a previously passed current-Actual duplicate check, and all required plan gates; generic outcomes also update the triage plan. Provider-reference identities converge pending/completed lifecycle notices, with message identity as fallback. Expenses remain negative, income remains positive, and the Actual import Adapter writes both uncleared. Duplicates stop, updates stay in review, and unknown prior reconciliation cannot be replaced by an imported-ID-only preview. Existing observe items still require confirmation. Historical scan items remain inspectable but cannot be claimed, confirmed, retried or dispatched.
...
11. `server/routes/briefing/transaction-imports.ts` — exposes owner-scoped per-email status, confirm, retry, and dismiss controls behind briefing cookie authentication; clients read durable item state and redacted planner evidence. Source configuration routes and client contracts are removed.

**Transfer schedule branch:** `server/transaction-imports/financial-email-transfer.ts` separates transfer jobs before expense grouping. Owner/source/destination/cents/date identity deduplicates reminder messages while the existing unique generic-import index admits one item. `server/actual/actual.ts:reconcileTransferSchedule` routes to the SDK worker under its session lock; it syncs current Actual data, checks exact signed schedules and linked transfer pairs, and returns schedule/transaction-specific outcomes. Preview captures the budget ID. A conditional durable attempted marker must succeed before one create dispatch with a deterministic schedule ID. All later claims with that marker reconcile only, even after timeouts, restarts, manual retries, or a due-date change. Deleted, partial, conflicting or unverifiable writes require review; they are not recreated. Only future-dated notices create schedules. A notice on or after its date without an existing match requires review and never becomes a completed transaction. Stored observe-only items are not promoted.

**Caches/state:** durable run/item tables originating in migration 041; Gmail provider page token on the run; Actual metadata caches and bills mirror invalidated only after a changed commit. Migration 056 allows automatic-safe generic expenses; migration 058 preserves those rows and admits positive automatic-safe generic income. Legacy mapping rows remain audit/recovery data only. Raw email HTML is never stored.
...
New financial arrivals have a separate global review projection: `GET /api/briefing/financial-events/review?offset=…` pages waiting/needs-review events once per event, plus unassociated retry documents with a financial candidate. It reads durable local state without fetching providers, invoking AI, or admitting Actual writes. Pending/settled/ignored and unclassified assessment failures are excluded. Financial activity exposes this queue even when Actual is disconnected, and Dashboard shows a count plus three direct links. `/finance?financial=list&source=managed&financialEmail=…` opens the existing managed completion form with live revision/ownership checks and a safe source email disclosure, including archived mail. Closing a confirmed form is not required for queue refresh; the chosen record remains visible while processing settles.

The authenticated app polls `GET /api/briefing/financial-events/review-changes?afterAt=…&afterId=…` in bounded updated-time pages, independent of queue pagination. Browser alerts honor existing Notification permission and link to Finance or the single changed record. Stable source/action/owner-confirmation keys and cursor state live in browser localStorage; a short read overlap catches timestamp ties and in-flight updates without re-alerting on unchanged retries. A record needing the same action again after a new owner confirmation can alert once more. Paused/provider/authentication/capture delays and attempted recovery stay visible but silent. Alerts require Setpoint to remain open; this is not server push or Discord delivery. The queue itself remains durable on the server. Demo projects fictional review items, returns an empty notification change feed, and never requests permission or persists delivery state.

`financial_event_changed` SSE events invalidate queue reads and managed Inbox status; explicit owner completion uses the same browser change signal. The financial foreground opens over the retained Dashboard, so its SSE stream remains mounted. Queue/notification polling and visibility refresh still cover missed events and visibility changes. Arrival-owned items retain their existing review/write owner below the new queue; retired historical scans are read-only.
...
Dashboard review previews open the shared foreground with exact import item and `recordRun` identity; the run filter remains independently encoded by `runId`. Compatible `reviewPending=1` Settings links remain available. Arrival-owned item confirmation/retry/dismiss endpoints remain the write owners. Source email evidence is available inside the financial record through the existing body-fetch and safe iframe seam. Demo endpoints project the same in-memory transaction seed as Calendar, and confirmed demo imports update that seed.
...
## Shared financial activity and original receipts

`GET /api/briefing/financial-activity` reads both processing owners in one application database snapshot before alias grouping, semantic filtering, counts and pagination. Exact event/document/import-item references remain resolvable independently of list pages; imports retain every run occurrence. Early document references continue resolving after event association, including when source email is unavailable. Saved history reads never synchronize Actual or admit work.

Migration 063 freezes source occurrence aliases and original imported identities on insertion. Original settlement atomically appends the first result receipt through source-table triggers; subsequent owner state changes cannot rewrite it. Historical backfill retains only previously saved evidence and labels it historical. Target bindings are budget + resource ID + role, not permanent ownership of a shared schedule.

Managed previews capture raw SDK object evidence in the immutable admitted operation. Utility schedule dispatch checks both the existing schedule fingerprint and the complete prepared raw graph. Ordinary legacy imports take a fresh preflight, durably admit its preparation, then compare it again in the SDK session before dispatch. An admitted legacy original import is subsequently reconciled without another import call. Successful synchronized results retain exact raw transaction IDs (including reciprocal/split rows) and schedule/rule/next-date graph IDs; raw database cents never come from SDK display previews. Unknown recovery provenance stays unknown.

`POST /api/briefing/financial-activity/binding` explicitly checks an older import's original imported ID and captured account in the selected Actual budget. A unique exact match can be saved as a binding with unknown original provenance; missing, ambiguous, unavailable and changed-budget outcomes remain explicit. No replacement or fuzzy match is created. Demo list/detail data stays in memory and binding inspection is inert. The correction facade supplies admission and corrected effective-result projection.

## Explicit corrections of completed financial activity

`POST /api/briefing/financial-corrections/preview` resolves one original/effective binding in its saved budget, synchronizes through the existing Actual worker, and freezes exact fields, resource IDs, provenance and source revisions. Confirmation synchronizes again under parent-process write coordination, rechecks source revisions transactionally, saves immutable ordered steps, and permanently guards only that source activity and its historical aliases. The authenticated status route reads the durable journal without provider work.

Existing scheduled transfers can be corrected through a `transfer_schedule` draft bound to the original exact schedule. The editor preserves transfer direction, recurrence and linked ledger history, resolves funding/destination accounts through Actual transfer payees, and requires a future date. Schedule edits do not post a transaction or create another schedule.

`POST /api/briefing/financial-corrections/recheck` explicitly synchronizes a stopped attention correction without dispatching another correction command. It verifies the last attempted effect against the preceding verified graph; every earlier effect must already be applied. Only a fully verified plan becomes completed. Conflicts retain attention and append observations, while verified partial work can continue through an explicit successor preview. A non-stopped or recovering attempt cannot be bypassed, and an unchanged remote read does not prove a lost command never wrote. Recheck does not accept a different Actual outcome as the intended result. A separate keep-preview/keep-confirm flow lets the owner explicitly accept the current result of a stopped attention correction. Its immutable preview binds the synchronized snapshot, correction revision and source revision. Confirmation reads Actual again and atomically records acceptance only if all remain unchanged. The completed journal retains original attempted step outcomes; its kept_actual observation and effective result contain the reviewed snapshot and observed values, not the unapplied draft. Missing or ambiguous records remain unsummarized. Repeated confirmation of the same review is inert, and later explicit corrections start from accepted current evidence. Neither review nor acceptance dispatches an Actual mutation.

The existing transaction-import runtime runs bounded correction recovery on startup, confirmation and its safety backstop. Each step records its attempt before one awaited internal SDK command. Ledger pairs use one explicit batch with automatic transfers and category learning disabled. Schedule creation has separate SDK commits; raw graph readback captures generated rule/date IDs and detects orphan prefixes. Local observations and post-sync observations are retained. Lost replies lead to observation of frozen IDs, never redispatch of an attempted step. Worker replacement waits for the retired process to exit before loading the SDK again.

A stopped, known partial operation can have an explicitly confirmed successor: retire an unchanged unreferenced orphan rule and allocate a fresh parent, reuse an already verified replacement schedule, or finish after a verified retirement. Unknown attempted effects cannot be superseded. Splits, reconciled balance changes, disposable counterpart provenance, recurring occurrence limitations, and due/past automatically posting schedules have explicit preflight constraints. Exact historical restoration additionally requires a saved one-time occurrence; keeping a current recurring schedule and linking the intended payment remains available.

Active correction journals conservatively block other Setpoint writes for that owner, including lightweight/manual writers. This temporary coordination does not reserve shared schedules permanently. Managed original recovery/admission/settlement and legacy scans/confirmation/dispatch cannot replay guarded source identities; a different order or later statement remains eligible. Shared activity and managed correlation/status use the last completed correction while retaining original receipts. Completion reuses Actual invalidation and financial publication; failed publication is retried independently.

Actual's field-level CRDT synchronization provides no remote compare-and-swap. Fresh snapshots and post-sync verification detect many conflicts but cannot isolate against simultaneous external/offline edits. An unverifiable or conflicting result stays recovering/attention; Setpoint never claims a distributed transaction. Demo correction inspection/preview/confirmation/status run against fictional in-memory examples, retaining original receipts. Managed completion also stays in memory.


## Shared financial foreground

Dashboard, Inbox, and notification references open focused financial content at `/finance` inside `WorkspaceRoute`. Legacy Settings workflow links redirect here with their exact targets. Settings retains financial preferences and connection repair. Historical scanning and manual statement capture are retired; future managed events accumulate source history. The retained controller owns list filters, selection, scroll and unsaved editing across responsive layouts and connection repair. All financial dialogs stay centered and size to their content within a viewport-dependent height cap. Width grows fluidly up to 1000px for records, 1680px for focused lists; financial record content stays within 960px. Focused lists split at 960px, with the list column growing from 300px to 400px. Smaller layouts retain the same selected record and form.

`FinancialRecord` presents saved/effective results and protected email evidence. Its Record history timeline orders linked email dates, immutable receipt captures, and saved updates from all admitted corrections, including predecessors. Detail reads add compact owner-scoped history in the same database snapshot; list payloads omit it. Unknown source dates stay explicitly undated, and latest correction polling refreshes its timeline entry without inventing intermediate transitions. Related-email counts identify one grouped record in Dashboard and the activity list. Managed pending completion uses `FinancialEventCompletionForm`; arrival-owned saved items use the original confirmation/retry/dismiss transports; historical scans cannot resume. Completed records use explicit synchronized read-only correction inspection, draft, exact frozen preview and confirmation. Status polling never replays a write. Uncertain confirmation responses use the preview identity to read admission status; attempted uncertain effects cannot be bypassed. Shared history refresh keeps a selected result even when it leaves the current filter.


## Financial settlement across consumers

Managed settlement and legacy batch settlement publish the existing `email_triage` / `financial_event_changed` event for saved source state. Correction completion retains its independent durable publication retry. The shared post-write invalidator first arms the durable 60-second bill mirror refresh, then refreshes Actual metadata, so a metadata failure cannot prevent the mirror retry. Verified recovery of an attempted arrival import follows the same invalidation path.

The bills provider includes `lastSuccessAt` in its change projection. After the mirror refresh, even ledger-only changes publish the existing `source: bills` event. The retained `useCurrentDashboard` invalidates the metadata singleton, marks Calendar bill/transaction ranges stale through the existing Dashboard event plan, and reloads current data. Immediate source-status events do not pretend the delayed mirror is already current. Dashboard finance coalesces both source and metadata invalidations, retaining a queued refresh when an event arrives during a read.

Shared history listens to financial publication and focus rather than a separate interval. The selected correction polls only unsettled status without overlapping requests and follows newer journal identities on activity refresh. Inbox managed and historical reads expose the latest correction state alongside the last verified effective result; original owner fields and receipts remain immutable. Corrected historical Recent Automation uses the effective payee and amount. Inbox correction status links to the exact shared record.

Build-time demo mutations update one exact-ID, in-memory financial projection. Managed completion, arrival confirmation and correction update ledger/bills, spending, current Dashboard, Inbox status and shared history. One local `ea-demo-financial-changed` event enters the same dashboard change handler without creating an EventSource. Transfer pairs remain inspectable in raw correction evidence but are excluded from the normal Calendar transaction range and spending, matching production. Duplicate confirmation is inert, refresh resets state, and no provider/network/persistence fallback exists.
...
Statement history reads persisted managed financial-event documents. There is no historical discovery, extraction preview, capture endpoint, or backfill writer. Missing source months remain unavailable; future events accumulate history after deployment.
