...
`financial_event_changed` SSE events invalidate queue reads and managed Inbox status; explicit owner completion uses the same browser change signal. Queue/notification polling and visibility refresh cover periods when the Dashboard SSE stream is unmounted in Settings. Historical imports retain their existing review/write owner below the new queue.
